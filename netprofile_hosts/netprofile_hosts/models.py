...
class Host(Base):
    ...
    group_id = Column(
        'hgid',
        UInt32(),
        ForeignKey('hosts_groups.hgid', name='hosts_def_fk_hgid',
                   onupdate='CASCADE'),
    ...
    entity_id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='hosts_def_fk_entityid',
    ...
    domain_id = Column(
        'domainid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='hosts_def_fk_domainid',
                   onupdate='CASCADE'),
        Comment('Domain ID'),
        nullable=False,
        info={
    ...
    original_id = Column(
        'aliasid',
        UInt32(),
        ForeignKey('hosts_def.hostid', name='hosts_def_fk_aliasid',
    ...
    created_by_id = Column(
        'cby',
        UInt32(),
        ForeignKey('users.uid', name='hosts_def_fk_cby',
    ...
    modified_by_id = Column(
        'mby',
        UInt32(),
        ForeignKey('users.uid', name='hosts_def_fk_mby',
                   ondelete='SET NULL', onupdate='CASCADE'),
...
class ServiceType(Base):
    ...
    def port_range(self):
        if (self.start_port
                and self.end_port
                and self.start_port <= self.end_port):
...
class Service(Base):
    ...
    __table_args__ = (
        Comment('Services'),
        Index('services_def_u_service', 'hostid', 'stid', 'domainid',
              unique=True),
    ...
    host_id = Column(
        'hostid',
        UInt32(),
        ForeignKey('hosts_def.hostid', name='services_def_fk_hostid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Host ID'),
        nullable=False,
        info={
    ...
    type_id = Column(
        'stid',
        UInt32(),
        ForeignKey('services_types.stid', name='services_def_fk_stid',
                   onupdate='CASCADE'),
        Comment('Service type ID'),
        nullable=False,
        info={
    ...
    domain_id = Column(
        'domainid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='services_def_fk_domainid',
                   ondelete='CASCADE', onupdate='CASCADE'),
...
class DomainService(Base):
    ...
    __table_args__ = (
        ...
        Index('domains_hosts_u_dhl', 'domainid', 'hostid', 'hltypeid',
              unique=True),
        ...
        Index('domains_hosts_i_hltypeid', 'hltypeid'),
        {
            ...
            'info':          {
                ...
                'create_wizard': SimpleWizard(
                                    title=_('Add new domain service'))
            }
        })
    id = Column(
        'dhid',
    ...
    domain_id = Column(
        'domainid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='domains_hosts_fk_domainid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Domain ID'),
        nullable=False,
        info={
    ...
    host_id = Column(
        'hostid',
        UInt32(),
        ForeignKey('hosts_def.hostid', name='domains_hosts_fk_hostid',
                   onupdate='CASCADE', ondelete='CASCADE'),
        Comment('Host ID'),
        nullable=False,
        info={
...
HostsAliasesView = View(
    ...
    ).select_from(Host).filter(Host.original_id.isnot(None)),
...
HostsRealView = View(
    ...
    DBSession.query(
        ...
        Host.modified_by_id.label('mby'),
        Host.description.label('descr')
    ).select_from(Host).filter(Host.original_id.is_(None)),
    check_option='CASCADED')
