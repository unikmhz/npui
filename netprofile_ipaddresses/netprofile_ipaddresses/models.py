...
class IPv4Address(Base):
    ...
    host_id = Column(
        'hostid',
        UInt32(),
        ForeignKey('hosts_def.hostid', name='ipaddr_def_fk_hostid',
    ...
    pool_id = Column(
        'poolid',
        UInt32(),
        ForeignKey('ippool_def.poolid', name='ipaddr_def_fk_poolid',
    ...
    network_id = Column(
        'netid',
        UInt32(),
        ForeignKey('nets_def.netid', name='ipaddr_def_fk_netid',
                   onupdate='CASCADE', ondelete='CASCADE'),
        Comment('Network ID'),
        nullable=False,
        info={
...
class IPv6Address(Base):
    ...
    host_id = Column(
        'hostid',
        UInt32(),
        ForeignKey('hosts_def.hostid', name='ip6addr_def_fk_hostid',
                   onupdate='CASCADE', ondelete='CASCADE'),
        Comment('Host ID'),
        nullable=False,
        info={
    ...
    pool_id = Column(
        'poolid',
        UInt32(),
        ForeignKey('ippool_def.poolid', name='ip6addr_def_fk_poolid',
                   onupdate='CASCADE', ondelete='SET NULL'),
        Comment('IP address pool ID'),
        nullable=True,
        default=None,
    ...
    network_id = Column(
        'netid',
        UInt32(),
        ForeignKey('nets_def.netid', name='ip6addr_def_fk_netid',
                   onupdate='CASCADE', ondelete='CASCADE'),
        Comment('Network ID'),
        nullable=False,
        info={
    ...
    host = relationship('Host',
                        innerjoin=True,
                        lazy='joined',
                        backref=backref('ipv6_addresses',
                                        cascade='all, delete-orphan',
                                        passive_deletes=True))
    pool = relationship('IPPool',
                        backref=backref('ipv6_addresses',
...
class IPv4ReverseZoneSerial(Base):
    ...
    def __str__(self):
        return '%s%02d' % (self.date.strftime('%Y%m%d'),
                           self.revision % 100)
    ...
    def zone_name(self):
        ...
        return '%d.%d.%d.in-addr.arpa' % ((ipint >> 8) % 256,
                                          (ipint >> 16) % 256,
                                          (ipint >> 24) % 256)
