...
def _action_probe_host(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe this host')),
            'itemId':  'probe'
        })
...
def _action_probe_entity(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe this entity\'s hosts')),
            'itemId':  'probe'
        })
...
def _action_probe_domain(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe hosts in this domain')),
            'itemId':  'probe'
        })
...
def _action_probe_house(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe hosts in this house')),
            'itemId':  'probe'
        })
...
def _action_probe_street(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe hosts on this street')),
            'itemId':  'probe'
        })
...
def _action_probe_district(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe hosts in this district')),
            'itemId':  'probe'
        })
...
def _action_probe_city(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe hosts in this city')),
            'itemId':  'probe'
        })
...
def _action_probe_housegroup(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe hosts in this house group')),
            'itemId':  'probe'
        })
...
def _action_probe_place(actions, req, model):
    if req.has_permission('HOSTS_PROBE'):
        ...
        actions.append({
            'iconCls': 'ico-netmon',
            'tooltip': loc.translate(_('Probe network devices located here')),
            'itemId':  'probe'
        })
...
def _probe_query_hosts(probe_type, ids, cfg, hm, queries):
    if probe_type == 'hosts':
        ...
        queries.append(DBSession().query(Host).join(
                with_polymorphic(Entity, Entity)).join(Address).filter(
                        Address.house_id.in_(ids)))
    elif probe_type == 'streets':
        queries.append(DBSession().query(Host).join(
                with_polymorphic(Entity, Entity)).join(Address).join(
                        House).filter(House.street_id.in_(ids)))
    elif probe_type == 'districts':
        queries.append(DBSession().query(Host).join(
                with_polymorphic(Entity, Entity)).join(Address).join(
                        House).join(Street).filter(
                                Street.district_id.in_(ids)))
    elif probe_type == 'cities':
        queries.append(DBSession().query(Host).join(
                with_polymorphic(Entity, Entity)).join(Address).join(
                        House).join(Street).filter(Street.city_id.in_(ids)))
    elif probe_type == 'housegroups':
        queries.append(DBSession().query(Host).join(
                with_polymorphic(Entity, Entity)).join(Address).join(
                        House).join(HouseGroupMapping).filter(
                                HouseGroupMapping.group_id.in_(ids)))
    elif probe_type == 'places':
        queries.append(DBSession().query(Host).join(NetworkDevice).filter(
                NetworkDevice.place_id.in_(ids)))
