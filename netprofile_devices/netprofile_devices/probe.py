...
HostProbeResult = namedtuple('HostProbeResult',
                             ('sent', 'returned',
                              'min', 'max', 'avg',
                              'detected'))
...
def _clone_with_detected(result, detected):
    return HostProbeResult(sent=result.sent,
...
class FPingProber(HostProber):
    def _arg(self, name, kwargs, defvalue=None):
        return kwargs.get(name,
                          self.cfg.get('netprofile.devices.fping.' + name,
                                       defvalue))

    def _arg6(self, name, kwargs, defvalue=None):
        return kwargs.get(name,
                          self.cfg.get('netprofile.devices.fping6.' + name,
                                       defvalue))
    ...
    def _fping_args(self, v6=False, **kwargs):
        ...
        if not os.access(fping_bin, os.X_OK):
            raise RuntimeError('Configured fping binary is not executable '
                               'by current user.')

        fping_interval = int(arg('packet_interval', kwargs, 12))
        fping_tgt_interval = int(arg('per_target_packet_interval',
                                     kwargs, 110))
    ...
    def _fping_run(self, addrs, v6=False, **kwargs):
        ...
        for outstr in stdout.splitlines():
            ...
            ret[addr.strip()] = HostProbeResult(
                ...
                max=rtt_max,
                avg=(None
                     if cnt_total == cnt_failed
                     else rtt_sum / (cnt_total - cnt_failed)),
    ...
    def _fping_addrs(self, addrs, ret, v6=False):
        ...
        if len(ping_addrs) > 0:
            for addrobj, proberes in self._fping_run(
                    ping_addrs.keys(), v6).items():
    ...
    def probe(self, hosts):
        ...
        for net in nets:
            ...
            gwq = sess.query(NetworkService).filter(
                ...
                NetworkService.type_id == 4).options(
                        joinedload(NetworkService.host))
            for service in gwq:
                # Get ifIndex of gateway interfaces
                for addr in (service.host.ipv4_addresses
                             + service.host.ipv6_addresses):
        ...
        for host, addrs in ret.items():
            for addr, proberes in addrs.items():
                ...
                if not proberes.detected:
                    ...
                    if netmgmt[mgmt].get_arp_entry(
                            gw_ifindex, addr.address) == 3:  # 3 = dynamic
