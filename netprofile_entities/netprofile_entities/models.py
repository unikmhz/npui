...
class EntityType(Base):
    ...
    __table_args__ = (
        ...
        Index('entities_types_u_plural', 'plural', unique=True),
        {
            ...
            'info':          {
                ...
                'form_view':     ('module', 'model', 'name', 'longname',
                                  'plural', 'root', 'leaf', 'descr'),
                'grid_hidden':   ('etypeid',),
    ...
    module_id = Column(
        'npmodid',
        UInt32(),
        ForeignKey('np_modules.npmodid', name='entities_types_fk_npmodid',
...
def _wizcb_ent_generic_next(wiz, em, step, act, val, req):
    ret = {'do': 'goto', 'goto': 'ent_physicalentity1'}
    if 'etypeid' in val:
...
class Entity(Base):
    ...
    def _filter_address(cls, query, value):
        ...
        if value.get('houseid'):
            ...
            if val > 0:
                query = query.join(Address).join(House).filter(
                        House.street_id == val)
        elif value.get('districtid'):
            val = int(value['districtid'])
            if val > 0:
                query = query.join(Address).join(House).join(Street).filter(
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_entities_def_ad'),
        {
            ...
            'info':          {
                ...
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'extra_search':  (TextFilter('phone', _filter_phone,
                                             title=_('Phone')),
                ...
                'create_wizard': Wizard(
                  ...
                  Step(
                      ...
                      id='ent_legalentity1',
                      title=_('Legal entity properties'),
                      on_prev='generic'),
                  Step(
                      ...
                      id='ent_legalentity2',
                      title=_('Legal entity contact person')),
                  Step(
                      ...
                      id='ent_legalentity3',
                      title=_('Legal entity details'),
                      on_submit=_wizcb_ent_submit('LegalEntity')),
                  Step(
                      # FIXME?
                      id='ent_structuralentity1',
                      title=_('Structure properties'),
                      on_prev='generic',
                  ...
                  Step(
                      ...
                      id='ent_externalentity1',
                      title=_('Misc. entity properties'),
                      on_prev='generic',
    ...
    parent_id = Column(
        'parentid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='entities_def_fk_parentid',
                   ondelete='CASCADE', onupdate='CASCADE'),
    ...
    state_id = Column(
        'esid',
        UInt32(),
        ForeignKey('entities_states.esid', name='entities_def_fk_esid',
                   onupdate='CASCADE'),
    ...
    type_id = Column(
        'etypeid',
        UInt32(),
        ForeignKey('entities_types.etypeid', name='entities_def_fk_etypeid',
                   onupdate='CASCADE'),
        Comment('Entity type'),
        nullable=False,
        info={
    ...
    created_by_id = Column(
        'cby',
        UInt32(),
        ForeignKey('users.uid', name='entities_def_fk_cby',
    ...
    modified_by_id = Column(
        'mby',
        UInt32(),
        ForeignKey('users.uid', name='entities_def_fk_mby',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Modified by'),
        nullable=True,
        default=None,
    ...
    created_by = relationship(
        ...
        backref=backref('created_entities',
                        passive_deletes=True))
    modified_by = relationship(
        'User',
        foreign_keys=modified_by_id,
        backref=backref('modified_entities',
                        passive_deletes=True))
    ...
    def __augment_result__(cls, sess, res, params, req):
        populate_related(res, 'state_id', 'state', EntityState,
                         sess.query(EntityState))
        populate_related_list(res, 'id', 'flagmap', EntityFlag,
                              sess.query(EntityFlag),
                              None, 'entity_id')
        populate_related_list(res, 'id', 'addresses', Address,
                              sess.query(Address),
                              None, 'entity_id')
        return res
    ...
    @property
    def data(self):
        return {'flags': [(ft.id, ft.name) for ft in self.flags]}
    ...
    def get_history(self, req, begin=None, end=None, category=None, max_num=20,
                    sort=None, sdir=None):
        hcat = 'entities.history.get.%s' % ('all'
                                            if category is None
                                            else category)
        hist = []
        req.run_hook(hcat,
        ...
        if sort == 'title':
            def sort_key(x): return x.title
        elif sort == 'author':
            def sort_key(x): return '' if x.author is None else x.author
        else:
            def sort_key(x): return x.time
        if sdir == 'DESC':
            sort_reverse = True
        else:
            sort_reverse = False
        hist = sorted(hist, key=sort_key, reverse=sort_reverse)
        ...
        Index('entities_flags_types_u_name', 'name', unique=True),
        {
            ...
            'info':          {
                ...
                'create_wizard': SimpleWizard(
                                    title=_('Add new entity flag type'))
            }
        })
    id = Column(
        'flagid',
...
class EntityFlag(Base):
    ...
    entity_id = Column(
        ...
        ForeignKey('entities_def.entityid',
                   name='entities_flags_def_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        nullable=False,
        info={
    ...
    type_id = Column(
        'flagid',
        UInt32(),
        ForeignKey('entities_flags_types.flagid',
                   name='entities_flags_types_fk_flagid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity flag type ID'),
        nullable=False,
        info={
...
class Address(Base):
    ...
    __table_args__ = (
        ...
        Index('addr_def_i_houseid', 'houseid'),
        {
            ...
            'info':          {
                ...
                'default_sort':  ({'property': 'houseid', 'direction': 'ASC'},
                                  {'property': 'flat', 'direction': 'ASC'}),
                'grid_view':     ('addrid', 'entity', 'primary', 'atype',
                                  'house', 'entrance', 'floor', 'flat',
                                  'descr'),
                'grid_hidden':   ('addrid',),
                'form_view':     ('entity', 'primary', 'atype',
                                  'house', 'entrance', 'floor', 'flat',
    ...
    entity_id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='addr_def_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        nullable=False,
        info={
    ...
    house_id = Column(
        'houseid',
        UInt32(),
        ForeignKey('addr_houses.houseid', name='addr_def_fk_houseid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('House ID'),
        nullable=True,
        default=None,
    ...
    def __str__(self):
        ...
        if self.entrance:
            ret.extend((loc(_('entr.')),
                        str(self.entrance)))
        if self.floor:
            ret.extend((loc(_('fl.')),
                        str(self.floor)))
        if self.flat:
            ret.extend((loc(_('app.')),
                        str(self.flat)))
...
class Phone(Base):
    ...
    __table_args__ = (
        ...
        Comment('Phone numbers'),
        {
            ...
            'info':          {
                ...
                'grid_view':     ('phoneid', 'entity', 'primary', 'ptype',
                                  'num', 'descr'),
                'grid_hidden':   ('phoneid',),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
    ...
    entity_id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='addr_phones_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        nullable=False,
        info={
    ...
    @property
    def data(self):
        return {'img': PhoneType.icon(self.type), 'str': str(self)}
...
class EntityFile(Base):
    ...
    entity_id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='entities_files_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        nullable=False,
        info={
    ...
    file_id = Column(
        'fileid',
        UInt32(),
        ForeignKey('files_def.fileid', name='entities_files_fk_fileid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('File ID'),
        nullable=False,
        info={
    ...
    file = relationship(
        'File',
        backref=backref('linked_entities',
                        cascade='all, delete-orphan',
                        passive_deletes=True))
...
class EntityComment(Base):
    ...
    entity_id = Column(
        ...
        UInt32(),
        ForeignKey('entities_def.entityid',
                   name='entities_comments_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        nullable=False,
        info={
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='entities_comments_fk_uid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=True,
        default=None,
    ...
    user = relationship(
        'User',
        backref=backref('entity_comments',
                        passive_deletes=True))
...
class EntityCommunicationChannel(Base):
                ...
                'grid_view':     ('ecommid',
                                  ...
                                  MarkupColumn(
                                      ...
                                      template='<a href="{uri:encodeURI}">'
                                               '{value:htmlEncode}</a>')),
                'grid_hidden':   ('ecommid',),
                'form_view':     ('type', 'entity', 'primary', 'scope',
                                  'value', 'descr'),
    ...
    type_id = Column(
        'commtid',
        UInt32(),
        ForeignKey('comms_types.commtid', name='entities_comms_fk_commtid',
                   ondelete='CASCADE', onupdate='CASCADE'),
    ...
    entity_id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='entities_comms_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        nullable=False,
        info={
    ...
    type = relationship(
        'CommunicationType',
        innerjoin=True,
        backref=backref('entity_channels',
                        cascade='all, delete-orphan',
                        passive_deletes=True))
...
class PhysicalEntity(Entity):
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_entities_physical_ad'),
        {
            ...
            'info':          {
                ...
                'grid_hidden':   ('entityid',),
                'form_view':     ('nick', 'parent', 'state', 'flags',
                                  'contractid',
                                  'name_family', 'name_given', 'name_middle',
                                  'gender',
                                  'email', 'homepage', 'birthdate',
                                  'pass_series', 'pass_num',
                                  'pass_issuedate', 'pass_issuedby',
                                  'descr'),
                'easy_search':   ('nick', 'name_family'),
                'extra_data':    ('grid_icon',),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'extra_search':  (TextFilter('phone', Entity._filter_phone,
                                             title=_('Phone')),
                                  AddressFilter('address',
                                                Entity._filter_address,
                                                title=_('Address'))),
                ...
                'create_wizard': Wizard(
                                    Step('nick', 'parent',
                                         'state', 'flags', 'descr',
                                         id='generic',
                                         title=_('Generic entity properties')),
                                    Step('contractid',
                                         'name_family', 'name_given',
                                         'name_middle', 'gender',
                                         id='ent_physical1',
                                         title=_('Person\'s name')),
                                    Step('pass_series', 'pass_num',
                                         'pass_issuedby', 'pass_issuedate',
                                         'email', 'homepage', 'birthdate',
                                         id='ent_physical3',
                                         title=_('Personal details')),
    ...
    id = Column(
        ...
        UInt32(),
        ForeignKey('entities_def.entityid',
                   name='entities_physical_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        primary_key=True,
        nullable=False,
...
class LegalEntity(Entity):
        ...
        Trigger('after', 'delete', 't_entities_legal_ad'),
        {
            ...
            'info':          {
                ...
                'grid_view':     (MarkupColumn(
                                  ...
                                  'entityid',
                                  'nick', 'name',
                                  'cp_name_family', 'cp_name_given'),
                'grid_hidden':   ('entityid',),
                'form_view':     ('nick', 'parent', 'state', 'flags',
                                  'contractid',
                                  'name',
                                  'cp_name_family', 'cp_name_given',
                                  'cp_name_middle', 'cp_title',
                                  'cp_email', 'homepage', 'address_legal',
                                  'props_inn', 'props_kpp', 'props_bic',
                                  'props_rs', 'props_cs', 'props_bank',
                                  'descr'),
                'easy_search':   ('nick', 'name'),
                'extra_data':    ('grid_icon',),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'extra_search':  (TextFilter('phone',
                                             Entity._filter_phone,
                                             title=_('Phone')),
                                  AddressFilter('address',
                                                Entity._filter_address,
                                                title=_('Address'))),
                ...
                'create_wizard': Wizard(
                                    Step('nick', 'parent',
                                         ...
                                         id='generic',
                                         title=_('Generic entity properties')),
                                    Step('contractid', 'name',
                                         'homepage',
                                         id='ent_legal1',
                                         title=_('Legal entity properties')),
                                    Step('cp_name_family', 'cp_name_given',
                                         'cp_name_middle', 'cp_title',
                                         'cp_email',
                                         id='ent_legal2',
                                         title=_('Legal entity contact '
                                                 'person')),
                                    Step('address_legal',
                                         'props_inn', 'props_kpp',
                                         'props_bic', 'props_rs',
    ...
    id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='entities_legal_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        primary_key=True,
        nullable=False,
...
class StructuralEntity(Entity):
    ...
    id = Column(
        ...
        UInt32(),
        ForeignKey('entities_def.entityid',
                   name='entities_structural_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        primary_key=True,
        nullable=False,
...
class ExternalEntity(Entity):
    ...
    id = Column(
        ...
        UInt32(),
        ForeignKey('entities_def.entityid',
                   name='entities_external_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Entity ID'),
        primary_key=True,
        nullable=False,
...
EntitiesBaseView = View(
    ...
    ).select_from(Entity.__table__).filter(Entity.parent_id.is_(None)),
