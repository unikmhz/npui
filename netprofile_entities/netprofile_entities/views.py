...
def new_entity_validator(ret, values, request):
    ...
    if not parent_id and not ent_type.root:
        ret['errors']['parent'].append(request.localizer.translate(
                _('This entity requires a parent.')))
    if parent_id:
        try:
...
@extdirect_method('Entity', 'get_history',
                  request_as_last_param=True, permission='ENTITIES_LIST')
