...
class Domain(Base):
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_domains_def_ad'),
        {
            ...
            'info':          {
                ...
                'form_view':     ('name', 'parent',
                                  'enabled', 'public', 'signed',
                                  'soa_refresh', 'soa_retry',
                                  'soa_expire', 'soa_minimum',
                                  'spf_gen', 'spf_rule', 'spf_errmsg',
                                  'dkim_name', 'dkim_data', 'dkim_test',
                                  'dkim_subdomains', 'dkim_strict',
                                  'dmarc_trailer',
                                  'descr'),
                'easy_search':   ('name', 'descr'),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),

                'create_wizard': Wizard(Step('name', 'parent', 'enabled',
                                             'public', 'signed', 'descr',
                                             title=_('Domain info')),
                                        Step('soa_refresh', 'soa_retry',
                                             'soa_expire', 'soa_minimum',
                                             'dkim_name', 'dkim_data',
                                             title=_('DNS options')),
    ...
    parent_id = Column(
        'parentid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='domains_def_fk_parentid',
                   onupdate='CASCADE'),
    ...
    def serial(self):
        ...
        return '%s%02d' % (self.serial_date.strftime('%Y%m%d'),
        ...
        while len(candidates) > 0:
            ...
            for domain, names, suffix in old_candidates:
                ...
                if domain is None or isinstance(domain, DomainAlias):
                    da_cond.append(and_(
                        DomainAlias.parent_id == (domain.id if domain
                                                  else None),
...
class DomainAlias(Base):
    ...
    parent_id = Column(
        'parentid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='domains_aliases_fk_parentid',
    ...
    domain_id = Column(
        'domainid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='domains_aliases_fk_domainid',
                   ondelete='CASCADE', onupdate='CASCADE'),
...
class DomainTXTRecord(Base):
    ...
    domain_id = Column(
        'domainid',
        UInt32(),
        ForeignKey('domains_def.domainid', name='domains_txtrr_fk_domainid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Domain ID'),
        nullable=True,
        default=None,
