...
class Module(ModuleBase):
    ...
    def get_sql_functions(cls):
        from netprofile_domains import models
        return (models.DomainGetFullFunction,)

    @classmethod
    def get_sql_views(cls):
        from netprofile_domains import models
        return (models.DomainsBaseView,
    ...
    def get_sql_data(cls, modobj, vpair, sess):
        ...
        sess.add(LogType(id=5,
                         name='Domains'))
        sess.flush()

        privs = (Privilege(code='BASE_DOMAINS',
                           name=_('Menu: Domains')),
                 Privilege(code='DOMAINS_LIST',
                           name=_('Domains: List')),
                 Privilege(code='DOMAINS_CREATE',
                           name=_('Domains: Create')),
                 Privilege(code='DOMAINS_EDIT',
                           name=_('Domains: Edit')),
                 Privilege(code='DOMAINS_DELETE',
                           name=_('Domains: Delete')),
                 Privilege(code='DOMAINS_SERVICETYPES_CREATE',
                           name=_('Domains: Create service types')),
                 Privilege(code='DOMAINS_SERVICETYPES_EDIT',
                           name=_('Domains: Edit service types')),
                 Privilege(code='DOMAINS_SERVICETYPES_DELETE',
        ...
        dstypes = (models.DomainServiceType(id=1,
                                            name=_('Primary Name Server'),
                                            unique=True),
                   models.DomainServiceType(id=2,
                                            name=_('Secondary Name Server'),
                                            unique=False),
                   models.DomainServiceType(id=3,
                                            name=_('Primary Mail Server'),
                                            unique=True),
                   models.DomainServiceType(id=4,
                                            name=_('Secondary Mail Server'),
                                            unique=False),
                   models.DomainServiceType(id=5,
                                            name=_('Default Host'),
                                            unique=False))
    ...
    def get_css(self, request):
        return ('netprofile_domains:static/css/main.css',)
