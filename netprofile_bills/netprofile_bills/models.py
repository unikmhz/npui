...
class Bill(Base):
	...
	__table_args__ = (
		...
		Trigger('after', 'delete', 't_bills_def_ad'),
		{
			...
			'info'          : {
				...
				'grid_view'     : ('billid', 'type', 'entity', 'stash', 'title', 'adate', 'ctime', 'mtime', 'state', 'ptime', 'sum', 'currency'),
				'grid_hidden'   : ('billid', 'type', 'stash', 'adate', 'ctime', 'mtime', 'ptime', 'currency'),
				'form_view'     : (
					'type', 'title', 'serial',
					'entity', 'stash', 'state',
					'currency', 'sum', 'adate',
	...
	type_id = Column(
		'btypeid',
		UInt32(),
		ForeignKey('bills_types.btypeid', name='bills_def_fk_btypeid', onupdate='CASCADE'), # ondelete=RESTRICT
		Comment('Bill type ID'),
		nullable=False,
		info={
			'header_string' : _('Type'),
	...
	entity_id = Column(
		'entityid',
		UInt32(),
		ForeignKey('entities_def.entityid', name='bills_def_fk_entityid', ondelete='CASCADE', onupdate='CASCADE'),
		Comment('Entity ID'),
		nullable=False,
		info={
	...
	stash_id = Column(
		'stashid',
		UInt32(),
		ForeignKey('stashes_def.stashid', name='bills_def_fk_stashid', ondelete='CASCADE', onupdate='CASCADE'),
		Comment('Stash ID'),
		nullable=False,
		info={
	...
	currency_id = Column(
		'currid',
		UInt32(),
		ForeignKey('currencies_def.currid', name='bills_def_fk_currid', onupdate='CASCADE'), # ondelete=RESTRICT
		Comment('Currency ID'),
		nullable=True,
		default=None,
		server_default=text('NULL'),
	...
	created_by_id = Column(
		'cby',
		UInt32(),
		ForeignKey('users.uid', name='bills_def_fk_cby', ondelete='SET NULL', onupdate='CASCADE'),
		Comment('Created by'),
		nullable=True,
		default=None,
		server_default=text('NULL'),
	...
	modified_by_id = Column(
		'mby',
		UInt32(),
		ForeignKey('users.uid', name='bills_def_fk_mby', ondelete='SET NULL', onupdate='CASCADE'),
		Comment('Modified by'),
		nullable=True,
		default=None,
		server_default=text('NULL'),
	...
	paid_by_id = Column(
		'pby',
		UInt32(),
		ForeignKey('users.uid', name='bills_def_fk_pby', ondelete='SET NULL', onupdate='CASCADE'),
