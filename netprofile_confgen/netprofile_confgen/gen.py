...
class ConfigGeneratorFactory(object):
    ...
    def outdir_files(self):
        if 'netprofile.confgen.files_output_dir' not in self.cfg:
            raise RuntimeError('File output directory for configuration '
                               'generator not defined in INI file.')
        outdir = self.cfg['netprofile.confgen.files_output_dir']
        if not os.path.isdir(outdir):
            if os.path.exists(outdir):
                raise RuntimeError('File output path exists but is '
                                   'not a directory.')
    ...
    def outdir_templates(self):
        if 'netprofile.confgen.templates_output_dir' not in self.cfg:
            raise RuntimeError('Template output directory for configuration '
                               'generator not defined in INI file.')
        outdir = self.cfg['netprofile.confgen.templates_output_dir']
        if not os.path.isdir(outdir):
            if os.path.exists(outdir):
                raise RuntimeError('Template output path exists but is '
                                   'not a directory.')
            os.mkdir(outdir, 0o750)
            logger.warn('Created confgen template output directory: %s',
                        outdir)
        return outdir
    ...
    def depdir_files(self):
        ...
        if deptype == 'puppet':
            return self.cfg.get(
                    'netprofile.confgen.puppet.files_dir',
                    '/etc/puppet/modules/npconfgen/files/generated')
    ...
    def depdir_templates(self):
        ...
        if deptype == 'puppet':
            return self.cfg.get(
                    'netprofile.confgen.puppet.templates_dir',
                    '/etc/puppet/modules/npconfgen/templates/generated')
    ...
    def srvdir_files(self, srv, xdir=None):
        ...
        if not os.path.isdir(srvdir):
            if os.path.exists(srvdir):
                raise RuntimeError('File output path for host "%s" exists '
        ...
        if not os.path.isdir(srvdir):
            if os.path.exists(srvdir):
                raise RuntimeError('File output path for host "%s" '
                                   'module "%s" exists but is not '
                                   'a directory.' %
                                   (host_name, srv.type.generator_name))
            os.mkdir(srvdir, 0o750)
        if xdir is not None:
            ...
            if not os.path.isdir(srvdir):
                if os.path.exists(srvdir):
                    raise RuntimeError(
                            'File output path for host "%s" module "%s" '
                            'directory "%s" exists but is not a directory.' %
                            (host_name, srv.type.generator_name, xdir))
                os.mkdir(srvdir, 0o750)
    ...
    def srvdir_templates(self, srv, xdir=None):
        ...
        if not os.path.isdir(srvdir):
            if os.path.exists(srvdir):
                raise RuntimeError('Template output path for host "%s" '
        ...
        if not os.path.isdir(srvdir):
            if os.path.exists(srvdir):
                raise RuntimeError(
                        'Template output path for host "%s" module "%s" '
                        'exists but is not a directory.' %
                        (host_name, srv.type.generator_name))
            os.mkdir(srvdir, 0o750)
        if xdir is not None:
    ...
    def get(self, gen_name):
        if gen_name not in self._gen:
            eps = list(pkg_resources.iter_entry_points(
                    ...
                    gen_name))
            if len(eps) < 1:
                raise RuntimeError('Unable to find configuration generator '
                                   'class "%s".' %
                                   (gen_name,))
            if len(eps) > 1:
                logger.warn('Multiple registrations found for '
                            'configuration generator class "%s".',
                            gen_name)
...
class BIND9Generator(ConfigGenerator):
    @reify
    def dns_srvs(self):
        return DBSession().query(Server).join(Server.type).filter(
                ServerType.generator_name.startswith('iscbind')).all()
    ...
    def dns_domain_services(self):
        ...
        return DBSession().query(DomainService).options(
                joinedload(DomainService.domain)).filter(
                        DomainService.type_id.in_((1, 2))).all()
    ...
    def domain_srv_rr(self, domain):
        ...
        return DBSession().query(Service).join(Host).filter(
                or_(Service.domain_id == domain.id,
                    and_(Service.domain_id.is_(None),
                         Host.domain_id == domain.id)))

    def revzone_ipv4(self, rz):
        from netprofile_networks.models import Network
        from netprofile_ipaddresses.models import IPv4Address
        return DBSession().query(IPv4Address).join(IPv4Address.host).join(
                IPv4Address.network).options(
                        contains_eager(IPv4Address.host),
                        contains_eager(IPv4Address.network)).filter(
                                (Network.ipv4_address + IPv4Address.offset
                                 >= int(rz.ipv4_network.network)),
                                (Network.ipv4_address + IPv4Address.offset
                                 <= int(rz.ipv4_network.broadcast)))

    def revzone_ipv6(self, rz):
        from netprofile_networks.models import Network
        from netprofile_ipaddresses.models import IPv6Address
        return DBSession().query(IPv6Address).join(IPv6Address.host).join(
                IPv6Address.network).options(
                        contains_eager(IPv6Address.host),
                        contains_eager(IPv6Address.network)).filter(
                                (Binary16ToDecimal(Network.ipv6_address)
                                 + IPv6Address.offset
                                 >= int(rz.ipv6_network.network)),
                                (Binary16ToDecimal(Network.ipv6_address)
                                 + IPv6Address.offset
                                 <= int(rz.ipv6_network.broadcast)))
    ...
    def domain_hosts(self, domain):
        ...
        # XXX: This is a hack, but it reduces poor ol' MySQL's crunch time
        #      by a factor of 10.
        sess = DBSession()

        hipv4 = dict((k, list(v)) for k, v in groupby(
            sess.query(IPv4Address).join(IPv4Address.host).filter(
                    Host.domain_id == domain.id),
            lambda ip: ip.host_id))
        hipv6 = dict((k, list(v)) for k, v in groupby(
            sess.query(IPv6Address).join(IPv6Address.host).filter(
                    Host.domain_id == domain.id),
            lambda ip: ip.host_id))

        for host in domain.hosts:
            attributes.set_committed_value(host, 'ipv4_addresses',
                                           hipv4.get(host.id, ()))
            attributes.set_committed_value(host, 'ipv6_addresses',
                                           hipv6.get(host.id, ()))
        return domain.hosts
    ...
    def generate_zone(self, param, ds, dname, outdir, tpl,
                      split_dns=False, is_root=False):
        if is_root:
        ...
        if split_dns:
            ...
            with open(os.path.join(outdir,
                                   dname + '.internal.zone'), 'wb') as fd:
                fd.write(tpl.render(**param))
            param['zonetype'] = 'external'
            with open(os.path.join(outdir,
                                   dname + '.external.zone'), 'wb') as fd:
                fd.write(tpl.render(**param))
        else:
            param.update({
                'domain':   ds.domain,
            ...
            with open(os.path.join(outdir,
                                   dname + '.generic.zone'), 'wb') as fd:
                fd.write(tpl.render(**param))

        if is_root:
            for alias in ds.domain.aliases:
                self.generate_zone(param, ds, str(alias), outdir, tpl,
                                   split_dns)
    ...
    def generate_revzone(self, param, rz, outdir, tpl, split_dns=False):
        ...
        if split_dns:
            ...
            with open(os.path.join(
                    outdir,
                    rz.zone_filename + '.internal.zone'), 'wb') as fd:
                fd.write(tpl.render(**param))
            if (not net.is_private
                ...
                with open(os.path.join(
                        outdir,
                        rz.zone_filename + '.external.zone'), 'wb') as fd:
                    fd.write(tpl.render(**param))
        else:
            param['zonetype'] = 'generic'
            with open(os.path.join(
                    outdir,
                    rz.zone_filename + '.generic.zone'), 'wb') as fd:
                fd.write(tpl.render(**param))
    ...
    def generate(self, srv):
        ...
        param = {
            ...
            'gen':     self,
            'srv':     srv,
            'deptype': self.confgen.cfg.get(
                           'netprofile.confgen.deployment_type', 'puppet'),
            'dtpl':    deptpl
        }
        conf_tpl = self.confgen.mako_lookup.get_template(
                'netprofile_confgen:templates/confgen/named.conf.mak')
        zone_tpl = self.confgen.mako_lookup.get_template(
                'netprofile_confgen:templates/confgen/named.zone.mak')

        with open(os.path.join(
                tpl_dir,
                'named.conf' + deptpl.file_suffix), 'wb') as fd:
        ...
        for ds in srv.host.domain_services:
            if ds.type_id != 1:
                continue
            self.generate_zone(param, ds, str(ds.domain), pridir, zone_tpl,
                               splitdns, True)
        ...
        if srv.get_bool_param('gen_revzones', True):
            ...
            rev_tpl = self.confgen.mako_lookup.get_template(
                    'netprofile_confgen:templates/confgen/named.revzone.mak')
...
class ISCDHCPGenerator(ConfigGenerator):
    ...
    def generate(self, srv):
        ...
        if dhcpv6:
            conf_tpl = self.confgen.mako_lookup.get_template(
                'netprofile_confgen:templates/confgen/dhcpd.ipv6.conf.mak')
        else:
            conf_tpl = self.confgen.mako_lookup.get_template(
                'netprofile_confgen:templates/confgen/dhcpd.ipv4.conf.mak')
