...
class DocumentResponse(Response):
	...
	"""
	def __init__(self, doc, docfmt, data, data_len=0, request=None, cache_max_age=None, content_encoding=None):
		...
		if filename is not None:
			...
			self.content_disposition = 'attachment; filename*=UTF-8\'\'%s' % (filename,)
...
class Document(Base):
	...
	def render(self, req, docfmt, tpl_vars):
		...
		if docfmt == 'pdf':
			...
			raise NotImplementedError('Requested format not implemented: %r' % (docfmt,))
