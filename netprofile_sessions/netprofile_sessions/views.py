...
@view_config(route_name='stashes.cl.accounts', name='sessions', context=Stash,
...
def client_sessions(ctx, request):
    ...
    total = sess.query(func.count('*')).select_from(cls).filter(
            cls.entity_id.in_(ent_ids),
            tsfield.between(ts_from, ts_to)).scalar()
    ...
    if page <= 0:
        ...
        page = max_page
    sessions = sess.query(cls).filter(
            cls.entity_id.in_(ent_ids),
            tsfield.between(ts_from, ts_to)).order_by(tsfield.desc())
    if total > per_page:
