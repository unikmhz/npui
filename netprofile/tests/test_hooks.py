...
class TestHooksAPI(unittest.TestCase):
	...
	def test_run_block_tpl(self, render):
		...
		self.hm.reg_block('b1', to)
		self.assertEqual(
			self.hm.run_block('b1', 1, 2, 3, request=req, req=req, item1=1, item2=2, item3=3),
			'BX'
		)

		render.assert_called_once_with(req, argv=(1, 2, 3), item1=1, item2=2, item3=3)
