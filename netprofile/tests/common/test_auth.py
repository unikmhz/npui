...
class TestHTTPDigestHeader(unittest.TestCase):
	...
	def test_is_valid_nonce(self):
		self.assertTrue(_is_valid_nonce('1234567890:0123456789ABCDEF:355087d404b3c5e7b6782df5ca930d1e', 'secret'))
		self.assertTrue(_is_valid_nonce('1234567890:FEDCBA9876543210:e376eccc2dd057250eadb8345772b385', 'secret'))

		self.assertFalse(_is_valid_nonce('1234567890:FEDCBA9876543210:e376eccc2dd057250eadb8345772b385', 'SECRET'))
		self.assertFalse(_is_valid_nonce('FEDCBA9876543210:e376eccc2dd057250eadb8345772b385', 'secret'))
		self.assertFalse(_is_valid_nonce('1234567890:e376eccc2dd057250eadb8345772b385', 'secret'))
		self.assertFalse(_is_valid_nonce('1234567890::e376eccc2dd057250eadb8345772b385', 'secret'))
		self.assertFalse(_is_valid_nonce('1234567890:0123456789ABCDEF:e376eccc2dd057250eadb8345772b385', 'secret'))

	@mock.patch('time.time')
	def test_is_valid_nonce_timestamp(self, mock_time):
		mock_time.return_value = 1234567890.0

		nonce = 'NOT-A-TIMESTAMP:0123456789ABCDEF:355087d404b3c5e7b6782df5ca930d1e'
	...
	def test_generate_digest_challenge(self, format_kvp, gen_nonce):
		...
		format_kvp.assert_called_once_with(realm='realm', qop='auth', nonce='NONCE', opaque='opaque', algorithm='MD5', stale='false')
...
class TestDigestAuthenticationPolicy(unittest.TestCase):
	def setUp(self):
		...
		self.pol = DigestAuthenticationPolicy('secret', self.callback, realm='realm')
	...
	def test_unauth_invalid_nonce_timestamp(self, mock_time):
		...
		self.req.authorization = ('Digest', {
			'username'  : 'testuser',
			'realm'     : 'realm',
			'nonce'     : '1234567290:0123456789ABCDEF:50396f1fd279bfc5b81af6146e3ad02d',
			'uri'       : '/index.html',
			'response'  : 'fe6636cdb8b1733ecca83dcb14b13323',
			'cnonce'    : '1f3f713c',
			'nc'        : '00000001',
	...
	def test_auth_invalid_nonce_timestamp(self, mock_time):
		...
		self.req.authorization = ('Digest', {
			'username'  : 'testuser',
			'realm'     : 'realm',
			'nonce'     : '1234568490:0123456789ABCDEF:0da457bab1f2c04de5bce45b3435c818',
			'uri'       : '/index.html',
			'response'  : 'fe6636cdb8b1733ecca83dcb14b13323',
			'cnonce'    : '1f3f713c',
			'nc'        : '00000001',
...
class TestPluginAuthenticationPolicy(unittest.TestCase):
	...
	def test_default_unauth_userid(self):
		...
		self.pol_default.unauthenticated_userid.assert_called_once_with(self.req)
	...
	def test_default_remember(self):
		...
		self.pol_default.remember.assert_called_once_with(self.req, 'princ', a=1, b=2)
	...
	def test_t1_remember(self):
		...
		self.pol.remember(self.req, 'princ', a=1, b=2)
		self.pol_t1.remember.assert_called_once_with(self.req, 'princ', a=1, b=2)
