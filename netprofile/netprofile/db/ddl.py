...
def visit_alter_table_alter_column_mysql(element, compiler, **kw):
    ...
    if const:
        spec += " " + const
    if col.comment:
        spec += " COMMENT " + compiler.sql_compiler.render_literal_value(
                col.comment, sqltypes.STRINGTYPE)
...
class Comment(SchemaItem):
    ...
    def _set_parent(self, parent):
        ...
        if isinstance(parent, Table):
            SetTableComment(parent, self.text).execute_at('after_create',
                                                          parent)
            ...
            def _set_col_comment(column, meta):
                SetColumnComment(column, text).execute_at('after_create',
                                                          column.table)
...
def visit_set_column_comment_mysql(element, compiler, **kw):
    spec = compiler.get_column_specification(
            element.column, first_pk=element.column.primary_key)
    const = " ".join(compiler.process(constraint)
                     for constraint
                     in element.column.constraints)
    if const:
        spec += " " + const
    ...
    return 'ALTER TABLE %s MODIFY COLUMN %s COMMENT %s' % (
        ...
        spec,
        compiler.sql_compiler.render_literal_value(element.text,
                                                   sqltypes.STRINGTYPE))
...
def ddl_fmt(ctx, obj):
    ...
    if isinstance(obj, dt.datetime):
        ...
        return compiler.sql_compiler.render_literal_value(
                date, sqltypes.STRINGTYPE)
    if isinstance(obj, str):
        return compiler.sql_compiler.render_literal_value(
                obj, sqltypes.STRINGTYPE)
    if isinstance(obj, int):
        return compiler.sql_compiler.render_literal_value(
                obj, sqltypes.INTEGERTYPE)
...
class SQLFunction(object):
    ...
    """
    def __init__(self, name, args=(), returns=None, comment=None,
                 reads_sql=True, writes_sql=True,
                 is_procedure=False, label=None):
        self.name = name
        self.args = args
    ...
    def _autogen_repr(self, context):
        return ('npd.%s(%r, args=[%s], returns=%s, comment=%r, '
                'reads_sql=%r, writes_sql=%r, '
                'is_procedure=%r, label=%r)' % (
                    self.__class__.__name__,
                    self.name,
                    ', '.join(arg._autogen_repr(context) for arg in self.args),
                    (_repr_type(self.returns, context)
...
class SQLEvent(object):
    ...
    """
    def __init__(self, name, sched_unit='month', sched_interval=1,
                 starts=None, preserve=True, enabled=True, comment=None):
        self.name = name
    ...
    def __repr__(self):
        return ('%s(%r, sched_unit=%r, sched_interval=%r, '
                'starts=%r, preserve=%r, enabled=%r, comment=%r)' % (
                    self.__class__.__name__,
                    self.name,
                    self.sched_unit,
                    self.sched_interval,
...
def visit_create_view(element, compiler, **kw):
    ...
    if isinstance(sel, Query):
        sel = sel.statement.compile(compile_kwargs={'literal_binds': True},
                                    dialect=compiler.dialect)
    else:
...
class View(SchemaItem):
    ...
    def _set_parent(self, parent):
        if isinstance(parent, Table):
            self.parent = parent
            CreateView(self.name,
                       self.select,
                       check_option=self.check).execute_at('after_create',
                                                           parent)
