...
#
# NetProfile: ColanderAlchemy wrapper
...
class SQLAlchemySchemaNode(colander.SchemaNode):
    ...
    def get_schema_from_column(self, prop, overrides):
        ...
        colander_type = getattr(column.type, 'colander_type', False)

        imperative_type = overrides.pop('typ', None)
        declarative_type = declarative_overrides.pop('typ', None)
        if (declarative_type is None) and colander_type:
            declarative_type = colander_type
        ...
        if imperative_type is not None:
        ...
        elif (colander_type is None) or (isinstance(column_type, _IGNORE_TYPES)):
            return None
            ...
            print(repr(column))
            print(repr(column.type))
            raise NotImplementedError('Unknown type: %r %r' % (column_type, column))
