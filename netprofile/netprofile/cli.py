...
class ShowModule(ShowOne):
    ...
    def take_action(self, args):
        ...
        if mod not in mm.modules:
            raise RuntimeError('Seems like module "%s" does not exist.' %
                               (args.name,))
        ...
        if not pkg or not pkg.has_metadata(
                pkg_resources.Distribution.PKG_INFO):
            raise RuntimeError('PKG-INFO not found')

        for line in pkg.get_metadata_lines(
                pkg_resources.Distribution.PKG_INFO):
            if ': ' in line:
                (k, v) = line.split(': ', 1)
                if k == 'Summary':
    ...
    def take_action(self, args):
        ...
        if args.name.lower() == 'all':
            ...
            for mod in mm.modules:
                if mod != 'core':
                    try:
                        self.app.hooks.run_hook('np.cli.module.install.before',
                                                self.app, mod, sess)
                        ret = mm.install(mod, sess,
                                         allow_upgrades=args.upgrade)
                        self.app.hooks.run_hook('np.cli.module.install.after',
                                                self.app, mod, sess, ret)
        ...
        self.app.hooks.run_hook('np.cli.module.install.before',
                                self.app, args.name, sess)
        ret = mm.install(args.name, sess, allow_upgrades=args.upgrade)
        self.app.hooks.run_hook('np.cli.module.install.after',
                                self.app, args.name, sess, ret)
        if isinstance(ret, bool):
            if ret:
                self.log.info('Module "%s" successfully installed.',
                              args.name)
                return
            raise RuntimeError('Module "%s" is already installed.' %
                               (args.name,))
    ...
    def take_action(self, args):
        ...
        if args.name.lower() == 'all':
            ...
            for mod in mm.modules:
                if mod != 'core':
                    try:
                        self.app.hooks.run_hook('np.cli.module.upgrade.before',
                                                self.app, mod, sess)
                        ret = mm.upgrade(mod, sess)
                        self.app.hooks.run_hook('np.cli.module.upgrade.after',
                                                self.app, mod, sess, ret)
                    except ModuleError as e:
                        if self.app.options.debug:
                            raise e
                        ...
                        if ret:
                            self.log.info(
                                'Module "%s" successfully upgraded.',
                                mod)
            if not is_ok:
        ...
        if isinstance(ret, bool):
            if ret:
                ...
                return
            raise RuntimeError('Module "%s" is already up to date.' %
                               (args.name,))
        raise RuntimeError('Unknown result.')
    ...
    def take_action(self, args):
        ...
        if args.name.lower() == 'all':
            for mod in mm.modules:
                if mm.is_installed(mod, sess):
                    if mod != 'core':
                        self.app.hooks.run_hook('np.cli.module.enable.before',
                                                self.app, mod, sess)
                        ret = mm.enable(mod)
                        self.app.hooks.run_hook('np.cli.module.enable.after',
                                                self.app, mod, sess, ret)
                        if ret:
                            self.log.info('Enabled module "%s".', mod)
                elif vlevel > 1:
                    self.log.info(
                        'Module "%s" is not installed, so can\'t enable.',
                        mod)
            self.log.info('All done.')
            return
        ...
        if isinstance(ret, bool):
            if ret:
                ...
                raise RuntimeError(
                    'Module "%s" wasn\'t found or is not installed.' %
                    (args.name,))
        raise RuntimeError('Unknown result.')
    ...
    def take_action(self, args):
        ...
        if args.name.lower() == 'all':
            for mod in mm.modules:
                if mm.is_installed(mod, sess):
                    if (mod != 'core'):
                        self.app.hooks.run_hook('np.cli.module.disable.before',
                                                self.app, mod, sess)
                        ret = mm.disable(mod)
                        self.app.hooks.run_hook('np.cli.module.disable.after',
                                                self.app, mod, sess, ret)
                        if ret:
                            self.log.info('Disabled module "%s".', mod)
                elif vlevel > 1:
                    self.log.info(
                        'Module "%s" is not installed, so can\'t disable.',
                        mod)
            self.log.info('All done.')
            return
    ...
    def get_parser(self, prog_name):
        kwargs_opts = {
            ...
            'tag': (
                ...
                dict(
                    ...
                    help='Arbitrary "tag" name '
                         '- can be used by custom env.py scripts.'
                )
            ),
            'head': (
                '--head',
            ...
            'splice': (
                ...
                dict(
                    action='store_true',
                    help='Allow a non-head revision '
                         'as the "head" to splice onto.'
                )
            ),
            'depends_on': (
                '-D', '--depends-on',
            ...
            'autogenerate': (
                ...
                dict(
                    ...
                    help='Populate revision script with candidate migration '
                         'operations, based on comparison of database '
                         'to model.'
                )
            ),
            'head_only': (
        ...
        for fn in [getattr(alembic_cmd, n) for n in dir(alembic_cmd)]:
            if (inspect.isfunction(fn) and fn.__name__[0] != '_'
    ...
    def take_action(self, args):
        ...
        if moddef != 'core':
            if not mm.preload(moddef):
                raise RuntimeError('Requested module "%s" can\'t be loaded.' %
                                   (moddef,))
        ...
        if not os.path.isdir(version_dir):
            ...
            if args.initial:
                ...
                raise RuntimeError('Can\'t find version directory: "%s".' %
                                   (version_dir,))
...
class Deploy(Command):
    ...
    def _assert_dir(self, deploy_dir, name):
        ...
        if not os.path.lexists(fdir):
            ...
            raise RuntimeError('Path exists but is not a directory: "%s".' %
                               (fdir,))
            ...
            with open(outfile, 'x') as outf:
                for line in inf:
                    ...
                    if m:
                        ...
                        if m:
                            key = m.group(2)
                            if (cur_section in replace
                                    and key in replace[cur_section]):
                                line = (m.group(1) + key + m.group(3)
                                        + replace[cur_section][key] + '\n')
                    outf.write(line)
        # This is racy
