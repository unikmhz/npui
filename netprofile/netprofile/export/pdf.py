...
class PDFExportFormat(ExportFormat):
    ...
    def options(self, req, name):
        ...
        return ({
            ...
            'store':          {
                ...
                'sorters': [{'property': 'value', 'direction': 'ASC'}],
                'fields':  ('id', 'value'),
                'data':    tuple({'id': k, 'value': loc.translate(v[0])}
                                 for k, v
                                 in PAGE_ORIENTATIONS.items())
            }
        }, {
            'name':           'pdf_hmargins',
    ...
    def export(self, extm, params, req):
        ...
        ss = req.pdf_styles
        if ss is None:
            raise RuntimeError('PDF subsystem is not configured. '
                               'See application .INI files.')
        ...
        table = LongTable(tuple(storyteller(data, fields, flddef,
                                            localizer=loc,
                                            model=extm, styles=ss)),
...
def storyteller(data, fields, flddef, localizer=None, model=None,
                styles=None, write_header=True):
    if model and localizer and write_header:
        yield tuple(
            Paragraph(localizer.translate(flddef[idx].header_string),
                      styles['table_header'])
            for idx, field
            in enumerate(fields))
    for row in data:
