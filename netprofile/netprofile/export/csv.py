...
class CSVExportFormat(ExportFormat):
    ...
    def options(self, req, name):
        ...
        return ({
            ...
            'grow':           True,
            'shrinkWrap':     True,
            'value':          (req.settings.get('core.locale.csv_charset')
                               or 'utf_8'),
            'allowBlank':     False,
            'forceSelection': True,
            'editable':       False,
            'store':          {
                ...
                'sorters': [{'property': 'value', 'direction': 'ASC'}],
                'fields':  ('id', 'value'),
                'data':    tuple({'id': k, 'value': v[1]}
                                 for k, v
                                 in csv_encodings.items())
            }
        })
    ...
    def export(self, extm, params, req):
        ...
        res.app_iter = csv_generator(data, fields, csv_dialect,
...
def csv_generator(data, fields, dialect, encoding='utf_8',
                  localizer=None, model=None, batch=100, write_header=True):
    cnt = 0
