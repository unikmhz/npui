...
class ModuleManager(object):
    ...
    def _load(self, req, mstack, activate=True):
        ...
        if moddef in self.loaded:
            ...
            if curversion not in modspec:
                logger.error(
                    'Module "%s" version %s is already loaded, '
                    'but required version is %s.',
        ...
        if activate and not self.is_installed(req, DBSession()):
            if moddef != 'core':
                logger.error(
                    'Can\'t load uninstalled/obsolete module "%s". '
                    'Please install/upgrade it first.',
                    moddef)
            return False
        ...
        try:
            ...
            logger.error(
                'Can\'t load module "%s". Verify installation and try again.',
                moddef)
            return False
        if not issubclass(modcls, ModuleBase):
            logger.error(
                'Module "%s" is invalid. Verify installation and try again.',
                moddef)
            return False
        ...
        for depmod in modcls.get_deps():
            ...
            if not self._load(depreq, mstack, activate=activate):
                logger.error(
                    'Can\'t load module "%s", '
                    'which is needed for module "%s".',
                    depmod, moddef)
                return False
        mod = modcls(self)
        if activate:
            self.loaded[moddef] = mod
        self.cfg.include(lambda conf: mod.add_routes(conf),
                         route_prefix='/' + moddef)
    ...
    def load_enabled(self):
        ...
        sess = DBSession()
        try:
            for mod in sess.query(NPModule).filter(
    ...
    def upgrade(self, req, sess):
        ...
        if 'core' not in self.loaded and moddef != 'core':
            raise ModuleError(
                'Unable to upgrade anything prior to loading core module.')
        if not self.is_installed(Requirement(moddef), sess):
            raise ModuleError(
                'Can\'t upgrade uninstalled module "%s", '
                'use install command instead.' %
                (moddef,))
    ...
    def uninstall(self, req, sess):
        ...
        if 'core' not in self.loaded and moddef != 'core':
            raise ModuleError(
                'Unable to uninstall anything prior to loading core module.')
    ...
    def assert_loaded(self, *mods):
        ...
        if len(not_loaded) > 0:
            raise ModuleError(
                'These modules aren\'t loaded, but are required: %s.' %
                (', '.join(not_loaded),))

    def _find_ep(self, moddef):
        if moddef in self.modules:
            return self.modules[moddef]
        match = list(pkg_resources.iter_entry_points('netprofile.modules',
                                                     moddef))
        if len(match) == 0:
            raise ModuleError('Can\'t find module: "%s".' % (moddef,))
        if len(match) > 1:
            raise ModuleError(
                'Can\'t resolve module to single distribution: "%s".' %
                (moddef,))
        ep = match[0]
        if ep.name != moddef:
            raise ModuleError(
                'Loaded module source "%s", but was asked to load "%s".' %
                (ep.name, moddef))
    ...
    def get_export_format(self, name):
        ...
        """
        eps = tuple(pkg_resources.iter_entry_points(
            'netprofile.export.formats', name))
        if len(eps) == 0:
