...
class JsonReprEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if (isinstance(obj, Response)
                and obj.content_type == 'application/json'):
...
class ExtDirectRouter(object):
    ...
    def add_action(self, action_name, **settings):
        ...
        ``accepts_files``: If true, this action will be declared as
                           formHandler in API
        ``session_checks``: If false, checks for session- and login-related
                            errors will not be performed
        ``permission``: The permission needed to execute the wrapped callable
        ``request_as_last_param``: If true, the wrapped callable will receive
                                   a request object as last argument
        """
    ...
    def _get_api_dict(self, request):
        return dict(url=request.application_url + '/' + self.router_path,
                    type='remoting',
        ...
        try:
            ...
            if self.expose_exceptions:
                ...
                if self.debug_mode and hasattr(request, 'pdtb_id'):
                    # If pyramid_debugtoolbar is enabled, generate
                    # an interactive page and include the URL to access it
                    # in the ExtDirect exception response text.
                    request.exc_info = sys.exc_info()
                    ret['message'] = 'Exception: traceback URL: %s' % (
                        request.route_url('debugtoolbar',
                                          subpath=(request.pdtb_id,
                                                   'exception')),)
            else:
...
class extdirect_method(object):
    ...
    def __init__(self,
        ...
        self._settings = dict(action=action,
...
def includeme(config):
    ...
    router_view_perm = settings.get(
            'netprofile.ext.direct.router_view_permission')
