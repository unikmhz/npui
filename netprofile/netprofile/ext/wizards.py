...
class DeclEnumWizardField(CustomWizardField):
    ...
    def __init__(self, name, enum, **kwargs):
        assert isinstance(enum, EnumMeta), ('Tried to generate a wizard '
                                            'field from something othen '
                                            'than an enum.')
        self.name = name
        self.enum = enum
        self.kw = kwargs
    ...
    def get_cfg(self, model, req, **kwargs):
        ...
        for field in self.fields:
            if isinstance(field, CustomWizardField):
                fcfg = field.get_cfg(model, req, **kwargs)
                if fcfg is not None:
                    if (not kwargs.get('use_defaults', False)
                            and 'value' in fcfg):
                        del fcfg['value']
                    if isinstance(fcfg, list):
                        step.extend(fcfg)
...
class Wizard(object):
    ...
    Generally viewed client-side on 'create' and other events.
    """
    def __init__(self, *args, **kwargs):
    ...
    def get_cfg(self, model, req, **kwargs):
        ...
        for step in self.steps:
            ...
            if scfg:
                ...
                res.append(scfg)
                idx += 1
        req.run_hook('np.wizard.cfg.%s.%s' % (model.model.__moddef__,
