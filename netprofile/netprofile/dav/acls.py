...
class DAVPrincipalValue(DAVValue):
    ...
    def __init__(self, ptype, href=None, prop=None):
        ...
        if ptype == self.PROPERTY and prop is None:
            raise ValueError(
                'Property name must be present for this principal type.')
        self.type = ptype
        self.href = href
        self.prop = prop
...
class DAVACEValue(DAVValue):
    def __init__(self, principal, grant=(), deny=(),
                 invert=False, protected=False):
...
def _parse_ace(el):
    ...
    if len(grant) and len(deny):
        raise DAVBadRequestError(
            'Invalid ACE in request: '
            'can\'t contain both grant and deny lists.')
    return DAVACEValue(
        principal,
        grant=grant,
        deny=deny,
