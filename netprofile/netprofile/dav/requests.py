    ...
    def __init__(self, req):
        ...
        if req.body:
            try:
                ...
                raise DAVBadRequestError('Invalid or not well-formed XML '
...
class DAVPropPatchRequest(DAVRequest):
    def __init__(self, req):
        ...
        if not req.body:
            raise DAVUnsupportedMediaTypeError('PROPPATCH method must be '
                                               'supplied with an XML '
                                               'request body.')
        try:
            self.xml = etree.XML(req.body)
        except etree.XMLSyntaxError:
        ...
        if not self.xml or self.xml.tag != dprops.PROPERTY_UPDATE:
            raise DAVUnsupportedMediaTypeError('PROPPATCH method must '
                                               'be supplied with an XML '
                                               'request body.')
...
class DAVMkColRequest(DAVRequest):
    def __init__(self, req):
        ...
        if req.body:
            if req.content_type not in {'application/xml', 'text/xml'}:
                raise DAVUnsupportedMediaTypeError('MKCOL method must be '
                                                   'supplied with an XML '
                                                   'request body.')
            try:
                self.xml = etree.XML(req.body)
            ...
            if not self.xml or self.xml.tag != dprops.MKCOL:
                raise DAVUnsupportedMediaTypeError('MKCOL method body must '
                                                   'contain mkcol root '
                                                   'XML tag.')
        else:
            self.xml = None
    ...
    def process(self):
        ...
        try:
            ...
            raise DAVBadRequestError('MKCOL method body must contain '
                                     'resource type property.')
        req = self.req
        ret = req.dav.make_collection(req, self.ctx, self.new_name,
                                      rtype, props)
        if ret is None:
...
class DAVLockRequest(DAVRequest):
    ...
    def _content(self, node):
        return (node.text or '') + ''.join(etree.tostring(el,
                                                          encoding='unicode')
                                           for el in node)
    ...
    def get_lock(self, cls):
        ...
        if len(self.xml.xpath('d:lockscope/d:exclusive',
                              namespaces=dprops.NS_MAP)) > 0:
