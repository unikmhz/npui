...
class DAVManager(object):
	def __init__(self, config):
		self.cfg = config
		self.lock_cls = None
		self.features = ['1', '2', '3', 'extended-mkcol']
		self.methods = [
		...
		self.principal_urls = ['users', 'groups']
		self.reports = {
			dprops.EXPAND_PROPERTY       : DAVExpandPropertyReport,
			dprops.PRINC_PROP_SEARCH     : None,
			dprops.PRINC_SEARCH_PROP_SET : None
		}
	...
	def get_ctx_path(self, ctx):
		return [el for el in ctx.get_uri()[1:]]
	...
	def get_path_props(self, req, ctx, pset, depth):
		ret = {}
		ret[ctx] = self.get_node_props(req, ctx, pset) # catch exceptions
		if depth:
			for ch in self.children(ctx):
				ret[ch] = self.get_node_props(req, ch, pset) # catch exceptions
		return ret

	def get_node_props(self, req, ctx, pset=None):
		...
		if len(pset) > 0:
			...
			if (dprops.LOCK_DISCOVERY in pset) and (dprops.LOCK_DISCOVERY not in props):
				locks = self.get_locks(self.get_ctx_path(ctx))
				props[dprops.LOCK_DISCOVERY] = DAVLockDiscoveryValue(req, locks, show_token=False)
			# Now, append report-related properties
			if (dprops.SUPPORTED_REPORT_SET in pset) and (dprops.SUPPORTED_REPORT_SET not in props):
				props[dprops.SUPPORTED_REPORT_SET] = DAVSupportedReportSetValue(self.supported_report_set(ctx))
			for pn in pset:
				if pn in props:
					ret[200][pn] = props[pn]
				elif not all_props:
