...
def _wizcb_aent_init(wizard, model, req):
    ...
    wizard.steps.append(Step(ExternalWizardField('AccessEntity',
                                                 'pwd_hashed'),
                             ExternalWizardField('AccessEntity',
                                                 'stash'),
                             ExternalWizardField('AccessEntity',
                                                 'rate'),
                             id='ent_accessentity1',
                             title=_('Access entity properties'),
                             on_prev='generic',
...
class AccessEntity(Entity):
        ...
        Trigger('after', 'delete', 't_entities_access_ad'),
        {
            ...
            'info':          {
                ...
                'form_view':     ('nick', 'parent', 'state', 'flags',
                                  'pwd_hashed', 'stash', 'rate', 'next_rate',
                                  # 'alias_of',
    ...
    id = Column(
        'entityid',
        UInt32(),
        ForeignKey('entities_def.entityid', name='entities_access_fk_entityid',
    ...
    stash_id = Column(
        'stashid',
        UInt32(),
        ForeignKey('stashes_def.stashid', name='entities_access_fk_stashid',
                   onupdate='CASCADE'),
    ...
    rate_id = Column(
        'rateid',
        UInt32(),
        ForeignKey('rates_def.rateid', name='entities_access_fk_rateid',
                   onupdate='CASCADE'),
    ...
    alias_of_id = Column(
        ...
        ForeignKey('entities_access.entityid',
                   name='entities_access_fk_aliasid',
                   ondelete='CASCADE', onupdate='CASCADE'),
    ...
    next_rate_id = Column(
        'nextrateid',
        UInt32(),
        ForeignKey('rates_def.rateid', name='entities_access_fk_nextrateid',
    ...
    ipv4_address_id = Column(
        'ipaddrid',
        UInt32(),
        ForeignKey('ipaddr_def.ipaddrid', name='entities_access_fk_ipaddrid',
                   ondelete='SET NULL', onupdate='CASCADE'),
    ...
    ipv6_address_id = Column(
        'ip6addrid',
        UInt64(),
        ForeignKey('ip6addr_def.ip6addrid',
                   name='entities_access_fk_ip6addrid',
                   ondelete='SET NULL', onupdate='CASCADE'),
    ...
    next_rate = relationship(
        ...
        backref=backref('pending_access_entities',
    ...
    ipv4_address = relationship(
        'IPv4Address',
        backref=backref('access_entities',
                        passive_deletes=True))
    ...
    def data(self, req):
        ...
        if self.quota_period_end:
            ret['qpend'] = format_datetime(self.quota_period_end,
    ...
    def access_state_string(self, req):
        ...
        return req.localizer.translate(
                AccessState.from_string(self.access_state).description)
    ...
    def change_password(self, newpwd, opts, request):
        self.password_hashed = hash_password(self.nick, newpwd,
                                             subject='accounts')
        self.password_ntlm = hash_password(self.nick, newpwd, scheme='ntlm',
                                           subject='accounts')
        self.password_crypt = hash_password(self.nick, newpwd, scheme='crypt',
                                            subject='accounts')
        self.password_plaintext = hash_password(self.nick, newpwd,
                                                scheme='plain',
                                                subject='accounts')
        if self.nick:
            self.password_ha1 = hash_password(self.nick, newpwd,
                                              scheme='digest-ha1',
                                              subject='accounts')
...
class PerUserRateModifier(Base):
    ...
    __table_args__ = (
        ...
        Index('rates_mods_peruser_u_mapping',
              'rmtid', 'entityid', 'rateid',
              unique=True),
        ...
        Trigger('before', 'insert', 't_rates_mods_peruser_bi'),
        {
            ...
            'info':          {
                ...
                'grid_view':     ('rmid', 'entity', 'rate', 'type',
                                  'enabled', 'l_ord'),
                'grid_hidden':   ('rmid',),
    ...
    type_id = Column(
        ...
        ForeignKey('rates_mods_types.rmtid',
                   name='rates_mods_peruser_fk_rmtid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=False,
        info={
    ...
    entity_id = Column(
        ...
        ForeignKey('entities_access.entityid',
                   name='rates_mods_peruser_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=False,
        info={
    ...
    rate_id = Column(
        ...
        UInt32(),
        Comment('Rate ID'),
        ForeignKey('rates_def.rateid', name='rates_mods_peruser_fk_rateid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=True,
        default=None,
        server_default=text('NULL'),
    ...
    type = relationship(
        'RateModifierType',
        innerjoin=True,
        backref=backref('per_user_modifiers',
                        cascade='all, delete-orphan',
                        passive_deletes=True))
    entity = relationship(
        'AccessEntity',
        innerjoin=True,
        backref=backref('rate_modifiers',
                        cascade='all, delete-orphan',
                        passive_deletes=True))
    rate = relationship(
        'Rate',
...
class AccessBlock(Base):
    ...
    entity_id = Column(
        ...
        Comment('Access entity ID'),
        ForeignKey('entities_access.entityid',
                   name='accessblock_def_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=False,
        info={
            'header_string': _('Account'),
...
class AccessEntityLink(Base):
    ...
    entity_id = Column(
        ...
        Comment('Access entity ID'),
        ForeignKey('entities_access.entityid',
                   name='entities_access_links_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=False,
        info={
    ...
    type_id = Column(
        ...
        UInt32(),
        Comment('Link type ID'),
        ForeignKey('entities_access_linktypes.ltid',
                   name='entities_access_links_fk_ltid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=False,
        info={
            'header_string': _('Type'),
...
class AccessEntityChange(Base):
    ...
    entity_id = Column(
        ...
        Comment('Access entity ID'),
        ForeignKey('entities_access.entityid',
                   name='entities_access_changes_fk_entityid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        nullable=False,
        info={
            'header_string': _('Entity'),
    ...
    user_id = Column(
        ...
        UInt32(),
        Comment('User ID'),
        ForeignKey('users.uid', name='entities_access_changes_fk_uid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        nullable=True,
        default=None,
        server_default=text('NULL'),
    ...
    old_rate_id = Column(
        ...
        UInt32(),
        Comment('Old rate ID'),
        ForeignKey('rates_def.rateid',
                   name='entities_access_changes_fk_rateid_old',
                   ondelete='SET NULL', onupdate='CASCADE'),
        nullable=True,
        default=None,
        server_default=text('NULL'),
    ...
    new_rate_id = Column(
        ...
        Comment('New rate ID'),
        ForeignKey('rates_def.rateid',
                   name='entities_access_changes_fk_rateid_new',
                   ondelete='SET NULL', onupdate='CASCADE'),
        nullable=True,
        default=None,
        server_default=text('NULL'),
    ...
    user = relationship(
        'User',
        backref=backref('access_entity_changes',
                        passive_deletes=True))
    old_rate = relationship(
        'Rate',
        lazy='joined',
        foreign_keys=old_rate_id)  # No backref
    new_rate = relationship(
        'Rate',
        lazy='joined',
        foreign_keys=new_rate_id)  # No backref

    def get_entity_history(self, req):
        loc = req.localizer
        eh = EntityHistory(self.entity,
                           loc.translate(_('Access entity "%s" changed')) %
                                        (str(self.entity)),
                           self.timestamp,
                           None if (self.user is None) else str(self.user))
        if self.password_changed:
            eh.parts.append(EntityHistoryPart(
                    'access:password',
                    loc.translate(_('Password was changed'))))
        if self.old_access_state != self.new_access_state:
            recognized = AccessState.values()
            if self.old_access_state in recognized:
                eh.parts.append(EntityHistoryPart(
                        'access:state_old',
                        loc.translate(AccessState.from_string(
                                self.old_access_state).description)))
            if self.new_access_state in recognized:
                eh.parts.append(EntityHistoryPart(
                        'access:state_new',
                        loc.translate(AccessState.from_string(
                                self.new_access_state).description)))
...
CheckAuthFunction = SQLFunction(
    'check_auth',
    args=(SQLFunctionArgument('name', Unicode(255)),
...
AcctAuthzProcedure = SQLFunction(
    'acct_authz',
    args=(InArgument('name', Unicode(255)),),
