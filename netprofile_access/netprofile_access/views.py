...
@view_config(route_name='access.cl.home',
             renderer='netprofile_access:templates/client_home.mak',
             permission='USAGE')
...
@notfound_view_config(vhost='client',
                      renderer='netprofile_access:templates/client_error.mak')
def client_notfound(request):
    request.response.status_code = 404
    tpldef = {'error': request.localizer.translate(_('Page Not Found'))}
    request.run_hook('access.cl.tpldef', tpldef, request)
    request.run_hook('access.cl.tpldef.error', tpldef, request)
    return tpldef


@forbidden_view_config(vhost='client',
                       renderer='netprofile_access:templates/client_error.mak')
    ...
    if not authenticated_userid(request):
        request.session.flash({
            ...
            'text':  request.localizer.translate(
                         _('You need to log in to access this page'))
        })
        return HTTPSeeOther(location=request.route_url('access.cl.login'))
    request.response.status_code = 403
    tpldef = {'error': loc.translate(_('Access Denied'))}
    request.run_hook('access.cl.tpldef', tpldef, request)
    request.run_hook('access.cl.tpldef.error', tpldef, request)
    return tpldef


@view_config(route_name='access.cl.chpass',
             renderer='netprofile_access:templates/client_chpass.mak',
             permission='USAGE')
def client_chpass(request):
    cfg = request.registry.settings
    loc = request.localizer
    min_pwd_len = int(
            cfg.get('netprofile.client.registration.min_password_length', 8))
    errors = {}
    if 'submit' in request.POST:
        ...
        if csrf != request.get_csrf():
            ...
            if not verify_password(user.nick, oldpass, user.password_hashed,
                                   subject='accounts'):
...
@view_config(route_name='access.cl.upload', xhr=True, request_method='GET',
             renderer='json', permission='USAGE')
...
@view_config(route_name='access.cl.upload', xhr=True, request_method='POST',
             renderer='json', permission='USAGE')
def client_upload(request):
    ...
    for fo in request.POST.getall('files'):
        ...
        signal = request.run_hook('access.cl.upload',
                                  obj, mode, request, sess, tpldef)
...
@view_config(route_name='access.cl.download', request_method='GET',
             permission='USAGE')
...
@view_config(route_name='access.cl.download', xhr=True,
             request_method='DELETE', renderer='json', permission='USAGE')
...
@view_config(route_name='access.cl.login',
             renderer='netprofile_access:templates/client_login.mak')
def client_login(request):
    ...
    can_recover = asbool(
            cfg.get('netprofile.client.password_recovery.enabled', False))
    ...
    if 'submit' in request.POST:
        ...
        if csrf == request.get_csrf() and login:
            sess = DBSession()
            q = sess.query(AccessEntity).filter(
                AccessEntity.nick == login,
                AccessEntity.access_state != AccessState.block_inactive.value)
            for user in q:
                if verify_password(user.nick, passwd, user.password_hashed,
                                   subject='accounts'):
...
@view_config(route_name='access.cl.register',
             renderer='netprofile_access:templates/client_register.mak')
def client_register(request):
    ...
    can_reg = asbool(cfg.get('netprofile.client.registration.enabled', False))
    must_verify = asbool(
            cfg.get('netprofile.client.registration.verify_email', True))
    must_recaptcha = asbool(
            cfg.get('netprofile.client.registration.recaptcha.enabled', False))
    min_pwd_len = int(
            cfg.get('netprofile.client.registration.min_password_length', 8))
    ...
    if 'submit' in request.POST:
        ...
        if len(errors) == 0:
            ...
            if 'user' not in errors:
                ...
                login_clash = sess.query(func.count('*')).select_from(
                        Entity).filter(Entity.nick == login).scalar()
                if login_clash > 0:
        ...
        if len(errors) == 0:
            ...
            if must_verify:
                ...
                rand_len = int(cfg.get(
                        'netprofile.client.registration.code_length', 20))
                queue_mail = asbool(cfg.get(
                ...
                request.run_hook('access.cl.tpldef.register.mail',
                                 tpldef, request)
                msg_text = Attachment(
                ...
                if queue_mail:
                    ...
                    mailer.send(msg)
            return HTTPSeeOther(
                    location=request.route_url('access.cl.regsent'))
    tpldef = {
        'cur_loc':        cur_locale,
        'comb_js':        comb_js,
...
@view_config(route_name='access.cl.regsent',
             renderer='netprofile_access:templates/client_regsent.mak')
...
@view_config(route_name='access.cl.activate',
             renderer='netprofile_access:templates/client_activate.mak')
def client_activate(request):
    ...
    if code and login and len(code) == rand_len:
        sess = DBSession()
        for link in sess.query(AccessEntityLink).options(
                joinedload(AccessEntityLink.entity)).filter(
                        AccessEntityLink.type_id == link_id,
                        AccessEntityLink.value == code):
            # TODO: implement code timeouts
            ent = link.entity
            if (ent.access_state == AccessState.block_inactive.value
                    and ent.nick == login):
...
@view_config(route_name='access.cl.restorepass',
             renderer='netprofile_access:templates/client_restorepass.mak')
def client_restorepass(request):
    ...
    loc = request.localizer
    cfg = request.registry.settings
    cfg_pr = make_config_dict(cfg, 'netprofile.client.password_recovery.')
    comb_js = asbool(cfg.get('netprofile.client.combine_js', False))
    can_rp = asbool(cfg_pr.get('enabled', False))
    change_pass = asbool(cfg_pr.get('change_password', True))
    must_recaptcha = asbool(cfg_pr.get('recaptcha.enabled', False))
    maillogin = asbool(cfg.get('netprofile.client.email_as_username', False))
            ...
            for acc in sess.query(AccessEntity).filter(
              ...
              AccessEntity.access_state != AccessState.block_inactive.value):
                ...
                while ent:
                    ...
                    ent = ent.parent
                if email == ent_email:
                    queue_mail = asbool(cfg_pr.get('mail_queue', False))
                    sender = cfg_pr.get('mail_sender')

                    if change_pass:
                        pwd_len = int(cfg_pr.get('password_length', 12))
                        new_password = get_salt_string(pwd_len)
                        acc.change_password(new_password, request.POST,
                                            request)
                    ...
                    request.run_hook('access.cl.tpldef.password_recovery.mail',
                                     tpldef, request)
                    msg_text = Attachment(
                        data=render('netprofile_access:templates'
...
@view_config(route_name='access.cl.restoresent',
             renderer='netprofile_access:templates/client_restoresent.mak')
...
@view_config(route_name='stashes.cl.accounts', name='chrate', context=Stash,
             request_method='POST', permission='USAGE')
...
def _tpldef_list_accounts(tpldef, req):
    ...
    tpldef['rates'] = sess.query(Rate).filter(Rate.user_selectable.is_(True))
...
def _aent_hist_changes(hist, ent, req, begin, end, max_num):
    ...
    qh = sess.query(AccessEntityChange).options(
            joinedload(AccessEntityChange.user)).filter(
                    AccessEntityChange.entity == ent)
