...
F_DEFAULT_FILES = (F_OWNER_READ
                   | F_OWNER_WRITE
                   | F_GROUP_READ
                   | F_GROUP_WRITE
                   | F_OTHER_READ)
F_DEFAULT_DIRS = (F_OWNER_ALL
                  | F_GROUP_ALL
                  | F_OTHER_READ
                  | F_OTHER_EXEC)
...
class NPModule(Base):
    ...
    def __repr__(self):
        return 'NPModule(%r,%r,%r,%r)' % (self.id, self.name,
...
class NPVariable(Base):
    ...
    def get_rw(cls, name):
        ...
        if var is None or var not in sess:
            cls._var_map[name] = sess.query(cls).filter(
                    cls.name == name).with_for_update().one()
    ...
    def get_ro(cls, name):
        ...
        if var is None or var not in sess:
            cls._var_map[name] = sess.query(cls).filter(
                    cls.name == name).with_for_update(read=True).one()
        return cls._var_map[name]
...
class IntervalTaskSchedule(TaskSchedule):
    ...
    def describe(self, req):
        ...
        if self.interval_unit == TaskIntervalUnit.microseconds:
            return loc.pluralize(
                'Every microsecond', 'Every ${num} microseconds',
                self.interval,
                domain='netprofile_core',
                mapping={'num': self.interval})
        return '-'
    ...
    def describe(self, req):
        cron_bits = (
            ('*' if self.crontab_minute is None
             else self.crontab_minute),
            ('*' if self.crontab_hour is None
             else self.crontab_hour),
            ('*' if self.crontab_day_of_month is None
             else self.crontab_day_of_month),
            ('*' if self.crontab_month is None
             else self.crontab_month),
            ('*' if self.crontab_day_of_week is None
             else self.crontab_day_of_week))
...
class Task(Base):
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_tasks_def_ad'),
        {
            ...
            'info':          {
                ...
                'grid_hidden':   ('taskid',),
                'form_view':     ('name', 'schedule', 'enabled', 'log',
                                  'proc', 'args', 'kwargs',
    ...
    schedule_id = Column(
        'beatschid',
        UInt32(),
        ForeignKey('tasks_schedules.beatschid',
                   name='tasks_def_fk_beatschid',
    ...
    created_by_id = Column(
        'cby',
        UInt32(),
        ForeignKey('users.uid', name='tasks_def_fk_cby',
    ...
    modified_by_id = Column(
        'mby',
        UInt32(),
        ForeignKey('users.uid', name='tasks_def_fk_mby',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Modified by'),
        nullable=True,
        default=None,
    ...
    created_by = relationship(
        ...
        backref=backref('created_tasks',
                        passive_deletes=True))
    modified_by = relationship(
        'User',
        foreign_keys=modified_by_id,
        backref=backref('modified_tasks',
                        passive_deletes=True))
    ...
    def new_result(self, celery_result=None):
        log = TaskLog(procedure=self.procedure,
    ...
    __table_args__ = (
        ...
        Index('tasks_log_i_uuid', 'uuid'),
        {
            ...
            'info':          {
                ...
                'menu_section':  _('Tasks'),
                'menu_name':     _('Results'),
                'default_sort':  ({'property': 'startts',
                                   'direction': 'DESC'},),
                'grid_view':     ('tasklogid', 'uuid', 'proc',
                                  'startts', 'state'),
                'grid_hidden':   ('tasklogid', 'uuid'),
                'form_view':     ('uuid', 'state',
                                  'proc', 'args', 'kwargs',
                                  'startts', 'finishts',
                                  'result', 'traceback'),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple')
...
def secpol_errors(checkpw, loc):
    ...
    if 'pw_ctype_min' in checkpw:
        errors.append(loc.translate(
            _('Password has not enough character types.')))
    if 'pw_ctype_max' in checkpw:
        errors.append(loc.translate(
            _('Password has too many character types.')))
    if 'pw_dict_check' in checkpw:
        errors.append(loc.translate(_('Password was found in a dictionary.')))
    if 'pw_hist_check' in checkpw:
        errors.append(loc.translate(
            _('You used this password not too long ago.')))
...
class User(Base):
        ...
        Trigger('after', 'delete', 't_users_ad'),
        {
            ...
            'info':          {
                ...
                'grid_view':     ('uid', 'login',
                                  ...
                                  'manager', 'group', 'enabled', 'state',
                                  'security_policy'),
                'grid_hidden':   ('uid', 'name_middle', 'manager',
                                  'security_policy'),
                'form_view':     ('login',
                                  'name_family', 'name_given', 'name_middle',
                                  'org', 'orgunit', 'title',
                ...
                'create_wizard': Wizard(Step('login', 'pwd_hashed', 'group',
                                             title=_('New user')),
                                        Step('name_family', 'name_given',
                                             'name_middle', 'enabled',
                                             'state',
                                             title=_('New user details')),
    ...
    security_policy_id = Column(
        'secpolid',
        UInt32(),
        ForeignKey('secpol_def.secpolid', name='users_fk_secpolid',
                   onupdate='CASCADE'),
    ...
    manager_id = Column(
        'managerid',
        UInt32(),
        ForeignKey('users.uid', name='users_fk_managerid',
                   ondelete='SET NULL', onupdate='CASCADE'),
    ...
    photo_id = Column(
        'phfileid',
        UInt32(),
        ForeignKey('files_def.fileid', name='users_fk_phfileid',
                   ondelete='SET NULL', onupdate='CASCADE', use_alter=True),
    ...
    def change_login(self, newlogin, opts, request):
        ...
        if getattr(self, 'mod_pw', False):
            self.password_ha1 = hash_password(self.login, self.mod_pw,
                                              scheme='digest-ha1')
    ...
    def change_password(self, newpwd, opts, request):
        ...
        self.password_plaintext = hash_password(self.login, newpwd,
                                                scheme='plain')
        if self.login:
            self.password_ha1 = hash_password(self.login, self.mod_pw,
                                              scheme='digest-ha1')
    ...
    def last_password_change(self):
        return DBSession().query(
                PasswordHistory).filter(
                        PasswordHistory.user == self).order_by(
                                PasswordHistory.timestamp.desc()).first()
            ...
            for sname, section in sections.items():
                for setting_name, setting in section.items():
                    ...
                    if fullname in self.settings:
                        ret[fullname] = setting.parse_param(
    ...
    def ldap_attrs(self, settings):
        ...
        if 'netprofile.ldap.orm.User.default_shell' in settings:
            ret['loginShell'] = settings.get(
                    'netprofile.ldap.orm.User.default_shell',
                    '/sbin/nologin')
        if len(dnlist) > 0:
            ret['memberOf'] = dnlist
        ...
        for ph in self.phones:
            for attr in PhoneType.ldap_attrs(ph.type):
                # FIXME: format phone as intl
                #        (probably using phonenumbers lib).
    ...
    def get_dav_history(self, sess, token_value):
        ...
        if self in sess.deleted:
            return (DAVHistory(collection_id=coll_id,
    ...
    def _get_vcard(self):
        ...
        card.add('prodid').value = '-//NetProfile//NetProfile DAV %s//EN' % (
                __version__,)
        # FIXME: track proper mtime
        card.add('rev').value = dt.datetime.now(
        ...
        if len(fname) == 0:
            fname = (self.login,)
        if self.id:
            card.add('uid').value = 'urn:npobj:user:%s:%u' % (inst_id,
                                                              self.id)
        ...
        if self.photo and self.photo.plain_mime_type in ('image/jpeg',):
            ...
            photo.encoded = True
            photo.value = base64.b64encode(
                    self.photo.get_data(sess=DBSession())).decode()
    ...
    @validates('name_family', 'name_given', 'name_middle', 'login',
               'organization', 'organizational_unit', 'title', 'description')
    ...
    def sync_token(self, value):
        try:
            ...
            sess = DBSession()
            userst = NPVariable(name='DAV:SYNC:PLUG:USERS',
                                integer_value=value)
            sess.add(userst)
...
def _del_user(mapper, conn, tgt):
    sess = DBSession()
    sess.query(UserACL).filter(
            UserACL.privilege_id.in_(sess.query(Privilege.id).filter(
                    Privilege.resource_class == 'NPUser')),
            UserACL.resource == tgt.id).delete(
...
class Group(Base):
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_groups_ad'),
        {
            ...
            'info':          {
                ...
                'menu_name':     _('Groups'),
                'default_sort':  ({'property': 'name', 'direction': 'ASC'},),
                'grid_view':     ('gid', 'name', 'parent',
                                  'security_policy', 'root_folder'),
                'grid_hidden':   ('gid',),
                'form_view':     ('name', 'parent', 'security_policy',
                                  'visible', 'assignable', 'root_folder'),
                'easy_search':   ('name',),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),

                'create_wizard': Wizard(Step('name', 'parent',
                                             'security_policy',
                                             title=_('New group data')),
                                        Step('visible', 'assignable',
                                             'root_folder',
                                             title=_('New group details')),
    ...
    parent_id = Column(
        'parentid',
        UInt32(),
        ForeignKey('groups.gid', name='groups_fk_parentid',
                   ondelete='SET NULL', onupdate='CASCADE'),
    ...
    security_policy_id = Column(
        'secpolid',
        UInt32(),
        ForeignKey('secpol_def.secpolid', name='groups_fk_secpolid',
                   onupdate='CASCADE'),
        Comment('Security policy ID'),
        nullable=True,
        default=None,
    ...
    root_folder_id = Column(
        'rootffid',
        UInt32(),
        ForeignKey('files_folders.ffid', name='groups_fk_rootffid',
                   ondelete='SET NULL', onupdate='CASCADE', use_alter=True),
...
def _del_group(mapper, conn, tgt):
    sess = DBSession()
    sess.query(GroupACL).filter(
            GroupACL.privilege_id.in_(sess.query(Privilege.id).filter(
                    Privilege.resource_class == 'NPGroup')),
            GroupACL.resource == tgt.id).delete(
                    synchronize_session=False)
...
class Privilege(Base):
    ...
    __table_args__ = (
        ...
        Index('privileges_i_npmodid', 'npmodid'),
        {
            ...
            'info':          {
                ...
                'grid_view':     ('privid', 'module', 'code', 'name',
                                  'guestvalue', 'hasacls', 'canbeset'),
                'grid_hidden':   ('privid', 'canbeset'),
                'form_view':     ('module', 'code', 'name', 'guestvalue',
                                  'hasacls', 'resclass'),
    ...
    module_id = Column(
        'npmodid',
        UInt32(),
        ForeignKey('np_modules.npmodid', name='privileges_fk_npmodid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('NetProfile module ID'),
        nullable=False,
        default=1,
...
class Capability(object):
    ...
    def id(cls):
        return Column(
            'capid',
            UInt32(),
            Sequence(cls.__tablename__ + '_capid_seq'),
    ...
    def privilege_id(cls):
        return Column(
            'privid',
            UInt32(),
            ForeignKey('privileges.privid',
                       name=cls.__tablename__ + '_fk_privid',
                       ondelete='CASCADE', onupdate='CASCADE'),
            Comment('Privilege ID'),
            nullable=False,
            info={
...
class GroupCapability(Capability, Base):
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='capabilities_groups_fk_gid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Group ID'),
        nullable=False,
        info={
...
class UserCapability(Capability, Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='capabilities_users_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
...
class ACL(object):
    ...
    def id(cls):
        return Column(
            'aclid',
            UInt32(),
            Sequence(cls.__tablename__ + '_aclid_seq'),
            Comment('ACL ID'),
            primary_key=True,
            nullable=False,
            info={
    ...
    def __str__(self):
        return '<%s(%s,%u) = %s>' % (str(self.__class__.__name__),
                                     str(self.code),
                                     str(self.resource),
                                     str(self.value))
...
class GroupACL(ACL, Base):
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='acls_groups_fk_gid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Group ID'),
        nullable=False,
        info={
...
class UserACL(ACL, Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='acls_users_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
...
class UserGroup(Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='users_groups_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='users_groups_fk_gid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Group ID'),
        nullable=False,
        info={
...
class SecurityPolicy(Base):
    ...
    __table_args__ = (
        ...
        Index('secpol_def_u_name', 'name', unique=True),
        {
            ...
            'info':          {
                ...
                'create_wizard': Wizard(Step('name', 'descr',
                                             title=_('Policy name')),
                                        Step('pw_length_min', 'pw_length_max',
                                             'pw_ctype_min', 'pw_ctype_max',
                                             'pw_dict_check', 'pw_dict_name',
                                             'pw_hist_check', 'pw_hist_size',
                                             title=_('Password '
                                                     'complexity checks')),
                                        Step('pw_age_min', 'pw_age_max',
                                             'pw_age_warndays',
                                             'pw_age_warnmail',
                                             'pw_age_action',
                                             title=_('Password age checks')),
                                        Step('net_whitelist', 'sess_timeout',
                                             'sess_window_ipv4',
                                             'sess_window_ipv6',
    ...
    def check_old_session(self, req, user, npsess, ts=None):
        ...
        if 'sess.nextcheck' in req.session:
            ...
            nextcheck = _sess_nextcheck(req, ts)
            req.session['sess.nextcheck'] = nextcheck
        if nextcheck < ts:
            # Expensive checks
            if not self.check_password_age(req, user, npsess, ts):
                return False
...
class CommunicationType(Base):
        ...
        Index('comms_types_i_impp', 'impp'),
        {
            ...
            'info':          {
                ...
                'create_wizard': SimpleWizard(
                        title=_('Add new communication type'))
            }
        })
    id = Column(
        'commtid',
    ...
    def grid_icon(self, req):
        ...
        return req.static_url('netprofile_core:static/img/comms/'
                              + icn + '.png')
...
class UserPhone(Base):
    ...
    __table_args__ = (
        ...
        Index('users_phones_i_num', 'num'),
        {
            ...
            'info':          {
                ...
                'menu_name':     _('Phones'),
                'default_sort':  ({'property': 'ptype', 'direction': 'ASC'},
                                  {'property': 'num', 'direction': 'ASC'}),
                'grid_view':     ('uphoneid', 'user', 'primary', 'ptype',
                                  'num', 'descr'),
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='users_phones_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    def __str__(self):
        ...
        return '%s: %s' % (loc(PhoneType.prefix(self.type)),
                           self.number)
...
class UserEmail(Base):
    ...
    __table_args__ = (
        ...
        Index('users_email_i_aliasid', 'aliasid'),
        {
            ...
            'info':          {
                ...
                'menu_name':     _('E-mail'),
                'default_sort':  ({'property': 'scope', 'direction': 'ASC'},
                                  {'property': 'addr', 'direction': 'ASC'},),
                'grid_view':     ('uemailid', 'user', 'primary', 'scope',
                                  'addr', 'original'),
                'grid_hidden':   ('uemailid',),
                'form_view':     ('user', 'primary', 'scope', 'addr',
                                  'original', 'descr'),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'create_wizard': SimpleWizard(
                        title=_('Add new e-mail address'))
            }
        })
    id = Column(
        'uemailid',
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='users_email_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    original_id = Column(
        'aliasid',
        UInt32(),
        ForeignKey('users_email.uemailid', name='users_email_fk_aliasid',
                   ondelete='CASCADE', onupdate='CASCADE'),
...
class UserCommunicationChannel(Base):
            ...
            'info':          {
                ...
                'grid_view':     ('ucommid',
                                  ...
                                  MarkupColumn(
                                      ...
                                      header_string=_('Address'),
                                      column_flex=3,
                                      template='<a href="{uri:encodeURI}">'
                                               '{value:htmlEncode}</a>')),
                'grid_hidden':   ('ucommid',),
                'form_view':     ('type', 'user', 'primary', 'scope',
                                  'value', 'descr'),
                'extra_data':    ('grid_icon', 'uri'),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'create_wizard': SimpleWizard(
                        title=_('Add new communication channel'))
            }
        })
    id = Column(
        'ucommid',
    ...
    type_id = Column(
        'commtid',
        UInt32(),
        ForeignKey('comms_types.commtid', name='users_comms_fk_commtid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Communication channel type ID'),
        nullable=False,
        info={
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='users_comms_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
...
class DAVLock(Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='dav_locks_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
    ...
    file_id = Column(
        'fileid',
        UInt32(),
        ForeignKey('files_def.fileid', name='dav_locks_fk_fileid',
                   ondelete='CASCADE', onupdate='CASCADE'),
...
class DAVHistory(Base):
    ...
    def find(cls, coll_id, since_token, until_token=None):
        ...
        q = sess.query(DAVHistory).filter(
                DAVHistory.collection_id == coll_id,
                DAVHistory.change_id > since_token).order_by(
                        DAVHistory.change_id)
...
class FileFolder(Base):
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_files_folders_ad'),
        {
            ...
            'info':          {
                ...
                'form_view':     ('name', 'user', 'group', 'rights',
                                  'ctime', 'mtime', 'descr'),
                'easy_search':   ('name',),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'extra_data':    ('allow_read', 'allow_write',
                                  'allow_traverse')
            }
        })
    id = Column(
    ...
    parent_id = Column(
        'parentid',
        UInt32(),
        ForeignKey('files_folders.ffid', name='files_folders_fk_parentid',
                   ondelete='CASCADE', onupdate='CASCADE'),
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='files_folders_fk_uid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Owner\'s user ID'),
        nullable=True,
        default=None,
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='files_folders_fk_gid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        ...
        if 'parentid' in values:
            ...
            if pid is None:
                ...
                if (not new_parent.can_write(u)
                        or not new_parent.can_traverse_path(u)):
                    return False
    ...
    def __getitem__(self, name):
        sess = DBSession()
        try:
            f = sess.query(FileFolder).filter(
                    FileFolder.parent == self,
                    FileFolder.name == name).one()
        except NoResultFound:
            try:
                f = sess.query(File).filter(
                        File.folder == self,
    ...
    def __acl__(self):
        ...
        return (
            (Allow if (rights & F_OWNER_EXEC) and can_access_u else Deny,
             ff_user, 'access'),
            (Allow if (rights & F_OWNER_READ) and can_access_u else Deny,
             ff_user, 'read'),
            (Allow if (rights & F_OWNER_WRITE) and can_access_u else Deny,
             ff_user, 'write'),
            (Allow if (rights & F_OWNER_EXEC) and can_access_u else Deny,
             ff_user, 'execute'),
            (Allow if (rights & F_OWNER_WRITE) and can_access_u else Deny,
            ...
            (Allow if (rights & F_GROUP_EXEC) and can_access_g else Deny,
             ff_group, 'access'),
            (Allow if (rights & F_GROUP_READ) and can_access_g else Deny,
             ff_group, 'read'),
            (Allow if (rights & F_GROUP_WRITE) and can_access_g else Deny,
             ff_group, 'write'),
            (Allow if (rights & F_GROUP_EXEC) and can_access_g else Deny,
             ff_group, 'execute'),
            (Allow if (rights & F_GROUP_WRITE) and can_access_g else Deny,
            ...
            (Allow if (rights & F_OTHER_EXEC) and can_access_o else Deny,
             Everyone, 'access'),
            (Allow if (rights & F_OTHER_READ) and can_access_o else Deny,
             Everyone, 'read'),
            (Allow if (rights & F_OTHER_WRITE) and can_access_o else Deny,
             Everyone, 'write'),
            (Allow if (rights & F_OTHER_EXEC) and can_access_o else Deny,
             Everyone, 'execute'),
            (Allow if (rights & F_OTHER_WRITE) and can_access_o else Deny,
    ...
    def dav_collection_id(self):
        if not self.id:
            raise RuntimeError('Requested collection ID '
                               'from non-persistent folder.')
        return 'FF:%u' % (self.id,)
    ...
    def get_dav_history(self, sess, token_value):
        ...
        if parent_hist.has_changes() or name_hist.has_changes():
            ...
            return (DAVHistory(collection_id=old_parent,
                               change_id=token_value,
                               is_collection=True,
                               operation=DAVHistoryOp.delete,
                               uri=old_name),
                    DAVHistory(collection_id=new_parent,
                               change_id=token_value,
                               is_collection=True,
                               operation=DAVHistoryOp.add,
                               uri=new_name))
...
class FileResponse(Response):
    def __init__(self, obj, request=None, cache_max_age=None,
        ...
        cr = None
        if (request.range
                and self in request.if_range
                and ',' not in request.headers.get('Range')):
        ...
        if request.method != 'HEAD':
            ...
            if data is None:
                ...
                if cr:
                    bio.seek(cr.start)
                    self.app_iter = WindowFileIter(bio, _BLOCK_SIZE,
                                                   cr.stop - cr.start)
                else:
                    if request is not None:
                        environ = request.environ
                        if 'wsgi.file_wrapper' in environ:
                            app_iter = environ['wsgi.file_wrapper'](
                                    bio, _BLOCK_SIZE)
                    if app_iter is None:
...
class File(Base):
    ...
    __table_args__ = (
        ...
        Trigger('after', 'delete', 't_files_def_ad'),
        {
            ...
            'info':          {
                ...
                'grid_view':     ('fileid', 'folder', 'fname', 'size',
                                  'ctime', 'mtime'),
                'grid_hidden':   ('fileid',),
                'form_view':     ('fname', 'folder', 'size',
                                  'user', 'group', 'rights',
                                  'ctime', 'mtime',
                                  'name', 'descr'),
                'easy_search':   ('fname', 'name'),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
                'extra_data':    ('allow_access', 'allow_read',
                                  'allow_write', 'allow_execute')
            }
        })
    id = Column(
        'fileid',
    ...
    folder_id = Column(
        'ffid',
        UInt32(),
        ForeignKey('files_folders.ffid', name='files_def_fk_ffid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Parent folder ID'),
        nullable=True,
        default=None,
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='files_def_fk_uid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Owner\'s user ID'),
        nullable=True,
        default=None,
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='files_def_fk_gid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Owner\'s group ID'),
        nullable=True,
        default=None,
    ...
    def dav_props(self, pset):
        ...
        if hasattr(self, '__req__'):
            ...
            if dprops.EXECUTABLE in pset:
                ret[dprops.EXECUTABLE] = ('T'
                                          if self.can_execute(req.user)
                                          else 'F')
        if dprops.LAST_MODIFIED in pset:
            ret[dprops.LAST_MODIFIED] = self.modification_time
        if isinstance(pset, DAVAllPropsSet):
    ...
    def set_region_from_file(self, infile, start, length,
                             user=None, sess=None):
        if sess is None:
            sess = DBSession()
        fd = -1
...
class FileChunk(Base):
    ...
    file_id = Column(
        'fileid',
        UInt32(),
        ForeignKey('files_def.fileid', name='files_chunks_fk_fileid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('File ID'),
        primary_key=True,
        nullable=False,
...
class VFSFileIO(io.BufferedIOBase):
    ...
    def fileno(self):
        raise io.UnsupportedOperation(errno.EBADF,
                                      'VFS objects don\'t have '
                                      'file descriptors',
                                      self.f)
    ...
    def truncate(self, sz=None):
        ...
        if sz < 0:
            raise IOError(errno.EINVAL,
                          'New file size can\'t be negative',
                          self.f)
        ...
        if self.mod_c:
            self.flush()
        if end_b == 0:
            self.sess.query(FileChunk).filter(
                    FileChunk.file_id == self.f.id,
                    FileChunk.offset >= end_c).delete()
        else:
            self.sess.query(FileChunk).filter(
                    FileChunk.file_id == self.f.id,
                    FileChunk.offset > end_c).delete()
    ...
    def read(self, maxb=-1):
        ...
        while read_sz > 0:
            ...
            retbuf[cursor:cursor + to_read] = mv[self.b_offset:self.b_offset
                                                 + to_read]
            read_sz -= to_read
            cursor += to_read
            self.b_offset += to_read
    ...
    def readinto(self, retbuf):
        ...
        while read_sz > 0:
            ...
            if self.c is None:
                to_read = min(read_sz, _CHUNK_SIZE - self.b_offset)
                retbuf[cursor:cursor + to_read] = (0 for _ in range(to_read))
                read_sz -= to_read
                cursor += to_read
                self.b_offset += to_read
                if self.b_offset >= _CHUNK_SIZE:
    ...
    def write(self, b):
        ...
        while write_sz > 0:
            ...
            if chunk_len < self.b_offset + to_write:
                ...
                self.buf[chunk_len:chunk_len + extend_sz] = (
                        0 for _ in range(extend_sz))
            mv = memoryview(self.buf)
            mv[self.b_offset:self.b_offset + to_write] = srcmv[cursor:cursor
                                                               + to_write]
            self.mod_c = True
            write_sz -= to_write
            cursor += to_write
...
class LogData(Base):
    ...
    type_id = Column(
        'type',
        UInt32(),
        ForeignKey('logs_types.ltid', name='logs_data_fk_type',
                   onupdate='CASCADE'),
    ...
    action_id = Column(
        'action',
        UInt32(),
        ForeignKey('logs_actions.laid', name='logs_data_fk_action',
                   onupdate='CASCADE'),
    ...
    def __str__(self):
        return '%s: [%s.%s] %s' % (str(self.timestamp),
                                   str(self.xtype),
                                   str(self.xaction),
                                   str(self.data))
...
class NPSession(Base):
    ...
    __table_args__ = (
        ...
        Index('np_sessions_i_lastts', 'lastts'),
        {
            ...
            'info':          {
                ...
                'default_sort':  ({'property': 'lastts',
                                   'direction': 'DESC'},),
                'grid_view':     ('npsid', 'sname', 'user', 'login',
                                  'startts', 'lastts', 'ipaddr', 'ip6addr'),
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='np_sessions_fk_uid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=True,
        default=None,
    ...
    def __augment_pg_query__(cls, sess, query, params, req):
        lim = query._limit
        if lim and lim < 50:
            return query.options(joinedload(NPSession.user))
        return query
...
class PasswordHistory(Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='users_pwhistory_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    if name:
        if inst_mm is None:
            ...
            if len(path) != 3:
                raise ValueError('Invalid setting name: %r' % (name,))
            try:
                setting = inst_mm.get_settings(
                        'global')[path[0]][path[1]][path[2]]
            except KeyError:
                return
...
class UserSetting(Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='np_settings_user_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
...
class DataCache(Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='datacache_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
...
class Calendar(Base):
    ...
    __table_args__ = (
        ...
        Index('calendars_def_i_gid', 'gid'),
        {
            ...
            'info':          {
                ...
                'default_sort':  ({'property': 'name', 'direction': 'ASC'},),
                'grid_view':     ('calid', 'name', 'user', 'group',
                                  'group_access', 'global_access'),
                'grid_hidden':   ('calid', 'user'),
                'form_view':     ('name', 'group',
                                  'group_access', 'global_access',
                                  'style', 'descr'),
                'easy_search':   ('name', 'descr'),
                'detail_pane':   ('netprofile_core.views', 'dpane_simple'),
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='calendars_def_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='calendars_def_fk_gid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Group ID'),
        nullable=True,
        default=None,
...
def _wizfld_import_cal(fld, model, req, **kwargs):
    return {
        ...
        'tpl':            '<tpl for=".">'
                          '<div class="x-boundlist-item">{Owner}: '
                          '{Title}</div></tpl>'
    }
...
class CalendarImport(Base):
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='calendars_imports_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    calendar_id = Column(
        'calid',
        UInt32(),
        ForeignKey('calendars_def.calid', name='calendars_imports_fk_calid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Calendar ID'),
        nullable=False,
        info={
...
class Event(Base):
    ...
    __table_args__ = (
        ...
        Trigger('before', 'update', 't_calendars_events_bu'),
        {
            ...
            'info':          {
                'menu_name':     _('Events'),
                'default_sort':  ({'property': 'dtstart',
                                   'direction': 'DESC'},),
                'grid_view':     ('evid', 'user', 'calendar', 'summary',
                                  'ctime', 'mtime', 'dtstart', 'dtend'),
                'grid_hidden':   ('evid', 'ctime', 'mtime'),
                'form_view':     ('user', 'calendar', 'summary',
    ...
    calendar_id = Column(
        'calid',
        UInt32(),
        ForeignKey('calendars_def.calid', name='calendars_events_fk_calid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Calendar ID'),
        nullable=False,
        info={
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='calendars_events_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
...
class AddressBook(Base):
    ...
    __table_args__ = (
        ...
        Index('abooks_def_i_synctoken', 'synctoken'),
        {
            ...
            'info':          {
                ...
                'default_sort':  ({'property': 'name', 'direction': 'ASC'},),
                'grid_view':     ('abookid', 'name', 'user', 'group',
                                  'group_access', 'global_access'),
    ...
    user_id = Column(
        'uid',
        UInt32(),
        ForeignKey('users.uid', name='abooks_def_fk_uid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('User ID'),
        nullable=False,
        info={
    ...
    group_id = Column(
        'gid',
        UInt32(),
        ForeignKey('groups.gid', name='abooks_def_fk_gid',
                   ondelete='SET NULL', onupdate='CASCADE'),
        Comment('Group ID'),
        nullable=True,
        default=None,
    ...
    def __getitem__(self, name):
        sess = DBSession()
        try:
            card = sess.query(AddressBookCard).filter(
                    AddressBookCard.address_book == self,
                    AddressBookCard.name == name).one()
        except NoResultFound:
            raise KeyError('No such file or directory')
    ...
    def __acl__(self):
        ...
        return (
            ...
            (Allow if self.group_access != CalendarAccess.none else Deny,
             ab_group, 'read'),
            (Allow if self.group_access == CalendarAccess.read_write else Deny,
             ab_group, 'write'),
            (Allow if self.group_access == CalendarAccess.read_write else Deny,
             ab_group, 'create'),
            (Allow if self.group_access == CalendarAccess.read_write else Deny,
             ab_group, 'delete'),
            (Allow if self.global_access != CalendarAccess.none else Deny,
             Everyone, 'read'),
            (Allow if self.global_access == CalendarAccess.read_write
             else Deny,
             Everyone, 'write'),
            (Allow if self.global_access == CalendarAccess.read_write
             else Deny,
    ...
    def dav_clone(self, req):
        # TODO: clone meta
        obj = AddressBook(name=self.name,
                          user_id=self.user_id,
                          group_id=self.group_id,
    ...
    def dav_collection_id(self):
        if not self.id:
            raise RuntimeError('Requested collection ID from non-persistent '
                               'address book.')
        return 'AB:%u' % (self.id,)
...
def _on_set_ab_synctoken(tgt, value, oldvalue, initiator):
    ...
    if tgt.user and tgt.user.id:
        try:
            ...
            var_abc = NPVariable(name='DAV:SYNC:ABC:%u' % (tgt.user.id,),
                                 integer_value=0)
        syncvars = (var_abc,
...
class AddressBookCard(Base):
    ...
    __table_args__ = (
        ...
        Index('abooks_cards_i_gid', 'name'),
        {
            ...
            'info':          {
                ...
                'default_sort':  ({'property': 'name', 'direction': 'ASC'},),
                'grid_view':     ('abookid', 'address_book', 'name',
                                  'ctime', 'mtime'),
                'grid_hidden':   ('abookid',),
    ...
    address_book_id = Column(
        'abookid',
        UInt32(),
        ForeignKey('abooks_def.abookid', name='abooks_cards_fk_abookid',
                   ondelete='CASCADE', onupdate='CASCADE'),
        Comment('Address book ID'),
        nullable=False,
        info={
    ...
    def __acl__(self):
        ...
        return (
            (Allow, ab_user, 'read'),
            (Allow, ab_user, 'write'),
            (Allow if ab.group_access != CalendarAccess.none else Deny,
             ab_group, 'read'),
            (Allow if ab.group_access == CalendarAccess.read_write else Deny,
             ab_group, 'write'),
            (Allow if ab.global_access != CalendarAccess.none else Deny,
             Everyone, 'read'),
            (Allow if ab.global_access == CalendarAccess.read_write else Deny,
             Everyone, 'write'),
            DENY_ALL)
    ...
    def dav_clone(self, req):
        # TODO: clone meta
        obj = AddressBookCard(name=self.name,
                              size=self.size,
                              etag=self.etag,
                              data=self.data)
        return obj
