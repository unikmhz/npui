...
Ext.define('Ext.ux.grid.menu.ListMenu', {
	...
	constructor: function(cfg)
	{
		...
		if(!cfg.store)
		{
			if(cfg.optStore)
			{
				this.store = Ext.create(cfg.optStore, {
					autoDestroy: true,
					buffered: false,
					pageSize: -1,
					listeners: {
			...
			else if(cfg.options)
			{
				...
				for(var i=0, len=cfg.options.length; i<len; i++)
				{
					...
					switch(Ext.type(value))
					{
						case 'array':
							...
							break;
						case 'object':
							options.push([value[this.idField], value[this.labelField]]);
							break;
						case 'string':
				...
				this.store = Ext.create('Ext.data.ArrayStore', {
					fields: [this.idField, this.labelField],
					data: options,
					listeners: {
						load: this.onLoad,
						scope: this
...
/*	show: function()
	...
	onLoad: function(store, records)
	{
		...
		{
			//itemValue = records[i].get(records[i].idProperty); //FIXME
