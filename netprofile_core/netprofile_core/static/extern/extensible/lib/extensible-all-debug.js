...
Ext.define('Extensible.calendar.form.EventWindow', {
    ...
    show: function(o, animateTarget) {
        ...
        if(me.animateTarget && me.animateTarget.getId)
           me.animateTarget = me.animateTarget.getId();
...
Ext.define('Extensible.calendar.view.AbstractCalendar', {
    ...
    prepareData: function() {
        ...
        var evtsInView = this.store.queryBy(function(rec) {
            return this.isEventVisible(rec);
        }, this);
    ...
    refreshAfterEventChange: function(action, operation) {
        ...
        var record = operation.getRecords()[0],
            ...
            isInstance = RInstanceStartDate && record && record.get && !!record.get(RInstanceStartDate.name),
    ...
    onRemove: function(store, operation) {
        ...
        if (this.enableFx && this.enableRemoveFx) {
            ...
            this.getEventEls(rec[Extensible.calendar.data.EventMappings.EventId.name]).remove();
    ...
    isEventVisible: function(evt) {
        var eventMappings = Extensible.calendar.data.EventMappings,
            ...
            calRec = this.calendarStore ? this.calendarStore.findRecord(calendarMappings.CalendarId.name,
                evt.get(eventMappings.CalendarId.name)) : null;

        if (calRec && calRec.get(calendarMappings.IsHidden.name) === true) {
        ...
        var start = this.viewStart.getTime(),
            ...
            evStart = evt.get(eventMappings.StartDate.name).getTime(),
            evEnd = evt.get(eventMappings.EndDate.name).getTime();
...
//        }
    ...
    handleEventMouseEvent: function(e, t, type) {
        ...
        if (el) {
            var rel_tgt = e.getRelatedTarget();
            
            if (el === Ext.fly(rel_tgt) || el.contains(Ext.fly(rel_tgt))) {
                return true;
            }
        ...
        if (t) {
            if (t.id && t.id.indexOf(this.dayElIdDelimiter) > -1) {
                var dt = this.getDateFromId(t.id, this.dayElIdDelimiter),
                    rel_tgt = e.getRelatedTarget(),
                    relTD, relDate;

                if (Ext.fly(rel_tgt)) {
                    relTD = Ext.fly(rel_tgt).is('td') ? rel_tgt : Ext.fly(rel_tgt).up('td', 3);
                    relDate = Ext.fly(relTD) && Ext.fly(relTD).id ? this.getDateFromId(Ext.fly(relTD).id, this.dayElIdDelimiter) : '';
                }
                if (!rel_tgt || dt !== relDate) {
    ...
    destroy: function() {
        ...
        Ext.destroyMembers(this, 'editWin', 'eventMenu', 'dragZone', 'dropZone');
    }
});
/**
